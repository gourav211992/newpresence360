@foreach($mrnItems as $key => $item)
    ...
    <tr id="row_{{$rowCount}}" data-index="{{$rowCount}}" @if($rowCount < 2 ) class="trselected" @endif>
        ...
        <td>
            ...
            <input type="hidden" class="form-control mw-100 mrn_qty" name="components[{{$rowCount}}][mrn_qty]" value="{{($qty - $item->purchase_bill_qty)}}" />
            <input type="number" class="form-control mw-100 accepted_qty text-end checkNegativeVal" name="components[{{$rowCount}}][accepted_qty]" value="{{($qty - $item->purchase_bill_qty)}}" readonly step="any"/>
        </td>
        <td>
            <input type="number" name="components[{{$rowCount}}][rate]" value="{{$item->rate}}" class="form-control mw-100 text-end rate checkNegativeVal" step="any"/>
            <input type="hidden" name="components[{{$rowCount}}][po_val]" value="{{$ItemRate}}" class="form-control mw-100 text-end po-rate checkNegativeVal" step="any"/>

        </td>
        <td>
            <input type="number" name="components[{{$rowCount}}][basic_value]" value="{{($qty - $item->purchase_bill_qty)*$item->rate}}"  class="form-control text-end mw-100 basic_value checkNegativeVal" readonly step="any"/>
            <input type="hidden" name="components[{{$rowCount}}][po_b_value]" value="{{($qty - $item->purchase_bill_qty)*$item->rate}}"  class="form-control text-end mw-100 basic_value checkNegativeVal" readonly step="any"/>
        </td>
        <td>
        ...
        <td>
            <input type="number" id="item_total_cost_{{$rowCount}}" name="components[{{$rowCount}}][item_total_cost]" value="{{($item->accepted_qty*$item->rate) - $item->discount_amount}}" readonly class="form-control mw-100 text-end item_total_cost" step="any"/>
            <input type="hidden" id="po_total_cost_{{$rowCount}}" name="components[{{$rowCount}}][po_total_cost]" value="{{($item->accepted_qty*$ItemRate) - $item->discount_amount}}" readonly class="form-control mw-100 text-end po_total_cost" step="any"/>
            @foreach($item->taxes as $tax_key => $item_tax)
                ...
                <input type="hidden" value="{{@$item_tax->ted_percentage}}" name="components[{{$rowCount}}][taxes][{{$tax_key + 1}}][t_perc]">
                <input type="hidden" value="{{@$item_tax->ted_amount}}" name="components[{{$rowCount}}][taxes][{{$tax_key + 1}}][t_value]">
            @endforeach
        </td>
        <td>
            <input type="number" name="components[{{$rowCount}}][item_variance]" value="" readonly class="form-control mw-100 text-end item_variance" step="any"/>
        </td>
        <td>
            <div class="d-flex">
