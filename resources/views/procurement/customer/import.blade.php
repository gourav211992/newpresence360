        ...
        <div class="content-header row">
            <div class="content-header-left col-md-5 mb-2">
                <div class="row breadcrumbs-top">
                    <div class="col-12">
                        <h2 class="content-header-title float-start mb-0">Upload Customer Data</h2>
            ...
            <div class="content-header-right text-sm-end col-md-7 mb-50 mb-sm-0">
                <div class="form-group breadcrumb-right">
                    <a href="{{ route('customer.index') }}" class="btn btn-secondary btn-sm mb-50 mb-sm-0">
                        <i data-feather="arrow-left-circle"></i> Back
                    </a>
                    <a href="{{ asset('templates/Customer_sample_template.xlsx') }}" class="btn btn-secondary btn-sm" id="download-template-btn" download>
        ...
        <div class="content-body">
            <section id="basic-datatable">
                <div class="row justify-content-center">
                    <div class="col-9">
                        <form class="importForm" method="POST" action="{{ route('customers.import.post') }}" enctype="multipart/form-data">
                            @csrf
...
<script>
    ...
    $(document).on('submit', '.importForm', function (e) {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                populateTable('#success-table-body', res.successful_customers);
                populateTable('#failed-table-body', res.failed_customers);
                $('#success-count-badge').text(`Records Succeeded: ${res.successful_customers.length}`);
                $('#success-count').text(`(${res.successful_customers.length})`);
                $('#failed-count').text(`(${res.failed_customers.length})`);
                $('#uploadProgress').hide();
                $('.hide-this-section').show();
                if (res.failed_customers.length > 0) {
                    $('.editbtnNew').show();  
                } else {
                    $('.editbtnNew').hide(); 
                }
                $('.exportBtn').on('click', function() {
                    ...
                    if (activeTab === '#Succeded') {
                        window.location.href = `/customers/export-successful-customers`; 
                    } else if (activeTab === '#Failed') {
                        window.location.href = `/customers/export-failed-customers`; 
                    }
                });
            },
    ...
    function populateTable(tableBodySelector, customers) {
                ...
                customers.forEach((customer, index) => {
                    const row = `
                        <tr>
                            ...
                            <td class="${customer.status === 'success' ? 'text-success' : 'text-danger'}">
                                ${customer.status === 'success' ? 'Success' : (customer.status === 'failed' ? 'Failed' : customer.remarks)}
                            </td>
                             <td class="${customer.remarks ? 'text-danger' : 'text-danger'}">
