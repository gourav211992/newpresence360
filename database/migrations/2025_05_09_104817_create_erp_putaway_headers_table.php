...
return new class extends Migration
{
    ...
    {
        # Put Away Headers
        foreach(['erp_putaway_headers', 'erp_putaway_headers_history'] as $tbl) {
        ...
        # Put Away Item
        foreach(['erp_putaway_details', 'erp_putaway_details_history'] as $tbl) {
            Schema::create($tbl, function (Blueprint $table) use ($tbl) {
                $table->id();
                if (str_contains($tbl, 'history')) {
                    $table->unsignedBigInteger('source_id')->nullable();
                ...
                if (str_contains($tbl, 'history')) {
                    $table->foreign('header_id')->references('id')->on('erp_putaway_headers_history')->onDelete('cascade');
                } else {
                    $table->foreign('header_id')->references('id')->on('erp_putaway_headers')->onDelete('cascade');
                }
            });
        }

        # Put Away Item Attributes
        foreach(['erp_putaway_attributes','erp_putaway_attributes_history'] as $tbl) {
            Schema::create($tbl, function (Blueprint $table) use ($tbl) {
                $table->id();
                if (str_contains($tbl, 'history')) {
                    $table->unsignedBigInteger('source_id')->nullable();
        ...
        # Put Away Item Locations
        foreach(['erp_putaway_item_locations','erp_putaway_item_locations_history'] as $tbl) {
            Schema::create($tbl, function (Blueprint $table) use ($tbl) {
                $table->id();
                if (str_contains($tbl, 'history')) {
                    $table->unsignedBigInteger('source_id')->nullable();
                ...
                $table->unsignedBigInteger('detail_id')->nullable();
                $table->unsignedBigInteger('item_id')->nullable();
                $table->string('packet_name', 291)->nullable();
                $table->string('packet_number', 291)->nullable();
                $table->string('storage_number', 291)->nullable();
                $table->unsignedBigInteger('store_id')->nullable();
                $table->unsignedBigInteger('sub_store_id')->nullable();
        ...
        # Put Away TED
        foreach(['erp_putaway_ted','erp_putaway_ted_history'] as $tbl) {
            Schema::create($tbl, function (Blueprint $table) use ($tbl) {
                $table->id();
                if (str_contains($tbl, 'history')) {
                    $table->unsignedBigInteger('source_id')->nullable();
        ...
        # Put Away Media
        Schema::create('erp_putaway_media', function (Blueprint $table) use ($tbl) {
            $table->id();
            $table->morphs('model');
    ...
    {
        Schema::dropIfExists('erp_putaway_media');
        Schema::dropIfExists('erp_putaway_ted_history');
        Schema::dropIfExists('erp_putaway_ted');
        Schema::dropIfExists('erp_putaway_item_locations_history');
        Schema::dropIfExists('erp_putaway_item_locations');
        Schema::dropIfExists('erp_putaway_attributes_history');
        Schema::dropIfExists('erp_putaway_attributes');
        Schema::dropIfExists('erp_putaway_details_history');
        Schema::dropIfExists('erp_putaway_details');
        Schema::dropIfExists('erp_putaway_headers_history');
        Schema::dropIfExists('erp_putaway_headers');
    }
};
