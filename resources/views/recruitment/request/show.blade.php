                        ...
                        <div class="form-group breadcrumb-right">
                            ...
                            @if ($jobRequest->status == App\Helpers\CommonHelper::PENDING && $user->id == $jobRequest->created_by)
                                <button class="btn btn-danger btn-sm mb-50 mb-sm-0" data-bs-target="#status-modal"
                                    data-bs-toggle="modal" data-status="{{ App\Helpers\CommonHelper::REVOKED }}"
                                    data-title="Revoked Request"><i data-feather="x-circle"></i> Revoke</button>
                            @endif
                            ...
                            @if (in_array($jobRequest->status, [App\Helpers\CommonHelper::PENDING, App\Helpers\CommonHelper::APPROVED_FORWARD]) &&
                                ...
                                @if ($user->manager_id)
                                    <button class="btn btn-primary btn-sm mb-50 mb-sm-0" data-bs-target="#status-modal"
                                        ...
                                        data-title="Approved Request" data-id="{{ $jobRequest->id }}">
                                ...
                                <button class="btn btn-success btn-sm mb-50 mb-sm-0" data-bs-target="#status-modal"
                                    data-bs-toggle="modal" data-status="{{ App\Helpers\CommonHelper::FINAL_APPROVED }}"
                                    data-title="Approved Request" data-id="{{ $jobRequest->id }}">
                                    <i data-feather="check-circle" class="me-50"></i>
                                ...
                                <button class="btn btn-danger btn-sm mb-50 mb-sm-0" data-bs-target="#status-modal"
                                    ...
                                    data-status="{{ App\Helpers\CommonHelper::REJECTED }}" data-title="Reject Request"
                                    data-id="{{ $jobRequest->id }}"><i data-feather="x-circle"></i> Reject</button>
                            @endif
                        </div>
                    </div>
                </div>
                        ...
                        <div class="col-12">
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    <div class="card">
                                        <div class="card-body customernewsection-form">
                                            <div class="exp-heade-top clickopendiv">
                                                <i data-feather="users"></i>
                                                <strong>Request Details</strong>
                                                <a href="#" class="open-req-sectab"><i data-feather="arrow-down"
                                                        class="float-end"></i></a>
                                                <a href="#" class="close-req-sectab text-danger"
                                                    style="display: none"><i data-feather='arrow-up-circle'
                                                        class="float-end"></i></a>
                                            </div>
                                            <div class="px-1 mt-2 reqinfodetail">
                                                <div class="row">
                                    ...
                                    @if ($job)
                                        <div class="card">
                                            <div class="card-body customernewsection-form">
                                                <div class="exp-heade-top clickjobdetdiv">
                                                    <i data-feather="users"></i> <strong>Job Details</strong>
                                                    <a href="#" class="open-job-sectab"><i
                                                            data-feather="arrow-down" class="float-end"></i></a>
                                                    <a href="#" class="close-job-sectab text-danger"
                                                        style="display: none"><i data-feather='arrow-up-circle'
                                                            class="float-end"></i></a>
                                                </div>
                                                <div class="px-1 mt-2 jobinfodetail">
                                                    <div class="row">
                                                        <div class="col-md-6 mb-50">
                                                            <label class="form-label">Job ID </label>
                                                            <h6 class="fw-bolder text-dark"><span
                                                                    class="badge rounded-pill badge-light-success badgeborder-radius font-small-2">{{ $job->job_id }}</span>
                                                            </h6>
                                                        </div>
                                                        <div class="col-md-6 mb-50">
                                                        ...
                                                        <div class="col-md-6 mb-50">
                                                            ...
                                                            <h6 class="fw-bolder text-dark">{{ $job->notice_peroid_id }}
                                                            </h6>
                                                        </div>
    ...
    <div class="modal fade" id="status-modal" tabindex="-1" aria-labelledby="statusModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <form id="status-form" role="post-data" method="POST"
                    action="{{ route('recruitment.requests.update-status', ['id' => $jobRequest->id]) }}"
                    redirect="{{ route('recruitment.requests.show', ['id' => $jobRequest->id]) }}">
                    <div class="modal-body pb-2">
                        <div class="row mt-1">
                            <div class="col-md-12">
                                <div class="mb-1">
...
@section('scripts')
    ...
    <script>
        $(document).on('click', '.clickopendiv .open-req-sectab', function(e) {
            e.preventDefault();
            let $container = $(this).closest('.clickopendiv');
            $container.next('.reqinfodetail').slideUp();
            $(this).hide();
            $container.find('.close-req-sectab').show();
        });

        $(document).on('click', '.clickopendiv .close-req-sectab', function(e) {
            e.preventDefault();
            let $container = $(this).closest('.clickopendiv');
            $container.next('.reqinfodetail').slideDown();
            $(this).hide();
            $container.find('.open-req-sectab').show();
        });


        $(document).on('click', '.clickjobdetdiv .open-job-sectab', function(e) {
            e.preventDefault();
            let $container = $(this).closest('.clickjobdetdiv');
            $container.next('.jobinfodetail').slideUp();
            $(this).hide();
            $container.find('.close-job-sectab').show();
        });

        $(document).on('click', '.clickjobdetdiv .close-job-sectab', function(e) {
            e.preventDefault();
            let $container = $(this).closest('.clickjobdetdiv');
            $container.next('.jobinfodetail').slideDown();
            $(this).hide();
            $container.find('.open-job-sectab').show();
        });
    </script>
@endsection
