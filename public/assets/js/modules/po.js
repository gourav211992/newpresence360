        ...
        if (totalItemValueAfterDisc) {
            row.find('[name*="[t_type]"]').each(function (taxIndex, TaxRow) {
                ...
                let tType = $(TaxRow)
                    .closest("td")
                    .find(
                        `[name*="components[${rowCount}][taxes][${
                            taxIndex + 1
                        }][t_type]"]`
                    )
                    .val();
...
function setTableCalculation() {
    ...
    if (
        ...
        $(".display_summary_discount_row")
            ...
            .each(function (index) {
                let oldValue =
                    $(`[name='disc_summary[${index + 1}][d_amnt]']`).val() || 0;
                oldValue = oldValue || eachDiscTypePrice;
                $(`[name="disc_summary[${index + 1}][d_amnt]"]`).closest(
                    "td"
                ).html(`${eachDiscTypePrice.toFixed(2)}
                <input type="hidden" value="${oldValue.toFixed(
                    2
                )}" name="disc_summary[${index + 1}][d_amnt]">
            `);
                // $(`[name="disc_summary[${index + 1}][d_amnt]"]`).val(eachDiscTypePrice.toFixed(2));
    ...
    Promise.all(taxPromises).then(() => {
        ...
        if ($(".display_summary_exp_row").find("[name*='[total]']").length) {
            $(".display_summary_exp_row")
                .find("[name*='[total]']")
                .each(function (index, eachItem) {
                    let eachExpTypePrice = Number($(eachItem).val()) || 0;

                    // Show value in table cell
                    $(`[name="exp_summary[${index + 1}][total]"]`).closest(
                        "td"
                    ).html(`
                    ${eachExpTypePrice.toFixed(2)}
                    <input type="hidden" value="${eachExpTypePrice.toFixed(2)}"
        ...
        $("#itemTable [id*='row_']").each(function (index, item6) {
            ...
            if (totalHeaderExp) {
                ...
                $(item6).find("[name*='[exp_amount_header]']").val("0.00");
            }
        });
    });
...
$(document).on("click", ".deleteItemDiscountRow", (e) => {
    ...
    if (!id) {
        ...
        $(`[name*="components[${rowCount}][discount_amount]"]`).after(
            hiddenDis
        );
        $("#disItemFooter #total")
            .attr("amount", totalAmnt)
            .text(totalAmnt.toFixed(2));
    }
    setTableCalculation();
});
        ...
        $("#itemTable #row_" + rowCount)
            ...
            .each(function (index, item) {
                let id = $(item)
                    .closest("td")
                    .find(
                        `[name='components[${rowCount}][delivery][${
                            index + 1
                        }][id]']`
                    )
                    .val();
                ...
                rowHtml += `<tr class="display_delivery_row">
                        ...
                        <td>
                            <input type="hidden" name="row_count" value="${rowCount}" id="row_count">
                            <input type="number" value="${dQty}" name="components[${rowCount}][delivery][${
                    index + 1
                }][d_qty]" class="form-control mw-100" />
                        </td>
                        <td>
                            <input type="date" min="${minDate}" name="components[${rowCount}][delivery][${
                    index + 1
                }][d_date]" value="${dDate}" class="form-control mw-100" /></td>
                        <td>
                        <a data-id="${id}" data-row-count="${rowCount}" data-index="${
                    index + 1
                }" href="javascript:;" class="text-danger deleteItemDeliveryRow"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-trash-2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg></a>
                       </td>
                    </tr>`;
            });
    }
...
$(document).on("click", ".addTaxItemRow", (e) => {
    ...
    if (
        !$("#deliveryScheduleTable [name*='[d_qty]']:last").val() &&
        $("#deliveryScheduleTable [name*='[d_qty]']").length
    ) {
        Swal.fire({
            title: "Error!",
            text: "Please enter quanity then you can add new row.",
    ...
    if (qty != itemQty) {
        let tblRowCount =
            $("#deliveryScheduleModal .display_delivery_row").length + 1;
        let rowHtml = `<tr class="display_delivery_row">
                            <td>${tblRowCount}</td>
                            <td>
                                <input type="hidden" name="row_count" value="${rowCount}" id="row_count">
        ...
        if (
            $("#deliveryScheduleModal").find(".display_delivery_row:last")
                .length
        ) {
            $("#deliveryScheduleModal")
                .find(".display_delivery_row:last")
                .after(rowHtml);
        } else {
            $("#deliveryScheduleModal").find("#deliveryFooter").before(rowHtml);
        }
...
$(document).on("click", ".itemDeliveryScheduleSubmit", (e) => {
    let isValid = true;
    document
        .querySelectorAll('input[name*="[d_qty]"], input[name*="[d_date]"]')
        .forEach((input) => {
            if (!input.value) {
                isValid = false;
    ...
    let rowCount = $("#deliveryScheduleModal .display_delivery_row")
        .find("#row_count")
        .val();
    let hiddenHtml = "";
    $("#deliveryScheduleTable .display_delivery_row").each(function (
        index,
        item
    ) {
        let dQty = $(item).find("[name*='d_qty']").val();
        let dDate = $(item).find("[name*='d_date']").val();
        hiddenHtml += `<input type="hidden" value="${dQty}" name="components[${rowCount}][delivery][${
            index + 1
        }][d_qty]"/>
                     <input type="hidden" value="${dDate}" name="components[${rowCount}][delivery][${
            index + 1
        }][d_date]" />`;
    });
    $("#itemTable #row_" + rowCount)
    ...
    if (!Number(id)) {
        ...
        setTimeout(() => {
            ...
            $(".display_delivery_row").each(function (index, item) {
                let a = `components[${rowCount}][delivery][${
                    index + 1
                }][d_qty]`;
                let b = `components[${rowCount}][delivery][${
                    index + 1
                }][d_date]`;
            ...
            $(
                `[name*='components[${rowCount}][delivery][${rowIndex}]']`
            ).remove();
...
$(document).on("click", "#add_new_item_dis", (e) => {
    ...
    let totalCost =
        parseFloat($(`[name*='components[${rowCount}][item_value]']`).val()) ||
        0;
    ...
    $(".display_discount_row").each(function (index, item) {
        ...
        <input type="hidden" value="${tedId}" name="components[${rowCount}][discounts][${
            index + 1
        }][ted_id]">
        <input type="hidden" value="${name}" name="components[${rowCount}][discounts][${
            index + 1
...
$(document).on(
    "input",
    'select[name*="[uom_id]"], input[name*="[qty]"]',
    function (e) {
    ...
    fetch(actionUrl).then((response) => {
        return response.json().then((data) => {
            ...
            if (data.status == 200) {
                ...
                $("#country_id").val(data?.data?.location_address?.country?.id);
                $("#state_id").val(data?.data?.location_address.state?.id);

                $("#party_country_id").val(
                    data?.data?.vendor_address?.country?.id
                );
                $("#party_state_id").val(data?.data?.vendor_address.state?.id);
                ...
                $(".vendor_address").text(
                    data?.data?.vendor_address?.display_address
                );
                $(".billing_address").text(
                    data?.data?.location_address?.display_address
                );
...
function copyItemRow() {
    ...
    try {
        ...
        $clone.closest("tr").find("input[name*='si_po_item_id']").val("");
        $clone.closest("tr").find("input[name*='po_item_id']").val("");
