...
class ExpenseHeader extends Model
{
    ...
    protected $table = 'erp_expense_headers';
    protected $fillable = [
        'organization_id',
        'group_id',
        'company_id',
    ...
    {
        return $this->hasOne(ExpenseHeaderHistory::class, 'header_id');
    }
    ...
    {
        return $this->hasMany(ExpenseTed::class, 'expense_header_id')->where('ted_level', 'H')->where('ted_type','Discount');
    }
    ...
    {
        return $this->hasMany(ExpenseTed::class,'expense_header_id')->where('ted_type', '=', 'Expense')
            ->where('ted_level', '=', 'H');
    }
    ...
    {
        return $this->hasMany(ExpenseTed::class,'expense_header_id');
    }

    public function expense_ted_tax()
    {
        return $this->hasMany(ExpenseTed::class,'expense_header_id')->where('ted_type','Tax');
    }
