@foreach($mrnItems as $key => $item)
    @php
        $rowCount = $key + 1;
        $qty = ($item->order_qty ?? 0.00) - ($item->accepted_qty ?? 0.00);
    @endphp
    <tr id="row_{{$rowCount}}" data-index="{{$rowCount}}" @if($rowCount < 2 ) class="trselected" @endif>
        ...
        <td>
            <input type="number" class="form-control mw-100 order_qty text-end checkNegativeVal" name="components[{{$rowCount}}][order_qty]" value="{{$item->order_qty}}" step="any"/>
        </td>
        <td>
        ...
        <td>
            <input type="number" class="form-control mw-100 rejected_qty text-end checkNegativeVal" name="components[{{$rowCount}}][rejected_qty]" readonly step="any"/>
        </td>
        <td>
            <div class="d-flex">
