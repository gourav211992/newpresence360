    ...
    <tbody>
        @forelse ($gstrData as $key => $item)
            <tr class="trail-bal-tabl-none">
                ...
                <td>{{ $item->revised_invoice_date ? App\Helpers\GeneralHelper::dateFormat3($item->revised_invoice_date) : '-' }}</td> 
                <td>{{ $item->invoice_amt ?  $item->invoice_amt : '-' }}</td>
                <td>{{ $item->port_code ? $item->port_code : '-' }}</td>
                <td>{{ $item->shipping_bill_no ? $item->shipping_bill_no : 0 }}</td> 
                <td>{{ $item->shipping_bill_date ? App\Helpers\GeneralHelper::dateFormat3($item->shipping_bill_date) : 0 }}</td> 
                <td>{{ $item->rate ? $item->rate.'%' : 0 }}</td> 
                <td>{{ $item->taxable_amt ? $item->taxable_amt : 0 }}</td> 
