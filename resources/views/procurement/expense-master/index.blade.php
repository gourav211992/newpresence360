        ...
        <div class="content-header pocreate-sticky">
            <div class="row">
                ...
                <div class="content-header-right text-end col-md-6">
                    <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#expenseMasterModal" id="addExpenseMasterBtn">
...
<!-- Modal -->
<div class="modal fade" id="expenseMasterModal" tabindex="-1" aria-labelledby="expenseMasterModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body px-sm-4 mx-50 pb-2">
                ...
                <form action="{{ route('expense-masters.store') }}" class="ajax-input-form" method="POST" id="expenseMasterForm">
                    @csrf
                       ...
                       <!-- Expense Ledger -->
                        <div class="col-md-6 mb-1">
                            ...
                            <div class="d-flex">
                                ...
                                <input type="hidden" id="ledger_id_expense" name="expense_ledger_id" class="ledger-id-expense">
                            </div>
                        </div>
                        ...
                        <div class="col-md-6 mb-1">
                            ...
                            <div class="d-flex">
                                ...
                                <input type="hidden" id="ledger_group_id_expense" name="expense_ledger_group_id" class="ledger-group-id-expense">
                            </div>
                        </div>
                        ...
                        <div class="col-md-6 mb-1">
                            ...
                            <div class="d-flex">
                                <input type="text" class="form-control autocomplete-ledger-service mw-100" 
                                    ...
                                    placeholder="Start typing to search for a ledger" autocomplete="off">
                                <input type="hidden" id="ledger_id_service_provider" name="service_provider_ledger_id" class="ledger-id-service">
                            </div>
                        </div>

                        <!-- Service Ledger Group -->
                        <div class="col-md-6 mb-1">
                            <label for="service_provider_ledger_group_id" class="form-label">Service Ledger Group</label>
                            <div class="d-flex">
                                <input type="text" class="form-control autocomplete-ledger-group-service mw-100" 
                                    ...
                                    placeholder="Start typing to search for a ledger group" autocomplete="off">
                                <input type="hidden" id="ledger_group_id_service_provider" name="service_provider_ledger_group_id" class="ledger-group-id-service">
                            </div>
                        </div>
            ...
            <div class="modal-footer justify-content-center">
                ...
                <button type="submit" class="btn btn-primary" id="submitBtn" form="expenseMasterForm">Submit</button>
            </div>
        </div>
    </div>
</div>
...
<script>
  ...
  function updateLedgerGroupAutocomplete(ledgerId, groupSelector, selectedGroupId = '') {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (Array.isArray(data)) {
                    $(groupSelector).autocomplete("option", "source", $.map(data, function(group) {
                        return {
                            label: group.name,
                            value: group.name,
                            id: group.id
    ...
    function showAllGroups(groupSelector) {
        $.ajax({
            ...
            success: function(data) {
                if (Array.isArray(data)) {
                    $(groupSelector).autocomplete("option", "source", $.map(data, function(item) {
                        return {
                            label: item.name,
                            value: item.name,
                            id: item.id
    ...
    $(".autocomplete-ledger-expense").each(function() {
        initializeAutocomplete(this, baseUrl + "/search/ledger", "#ledger_id_expense", '.autocomplete-ledger-group-expense');
    });

    $(".autocomplete-ledger-service").each(function() {
        initializeAutocomplete(this, baseUrl + "/search/ledger", "#ledger_id_service_provider", '.autocomplete-ledger-group-service');
    });
    ...
    $(".autocomplete-ledger-group-service").each(function() {
        initializeAutocomplete(this, baseUrl + "/search/group", "#ledger_group_id_service_provider");
    });
    ...
    function initializeLedgerGroupsOnPageLoad() {
        ...
        if (expenseLedgerId) {
            updateLedgerGroupAutocomplete(expenseLedgerId, '.autocomplete-ledger-group-expense', expenseLedgerGroupId);
        }
        ...
        if (serviceLedgerId) {
            updateLedgerGroupAutocomplete(serviceLedgerId, '.autocomplete-ledger-group-service', serviceLedgerGroupId);
        }
  }
    $(document).on('click', '.edit-btn', function() {
        ...
        const service_provider_ledger_group_name = $(this).data('service_provider_ledger_group_name');
        ...
        if (dt_basic_table.length) {
            var dt_discount_master = dt_basic_table.DataTable({
                ...
                columns: [
                    ...
                    { data: 'expense_ledger_id', render: renderData },
                    { data: 'service_provider_ledger_id', render: renderData },
