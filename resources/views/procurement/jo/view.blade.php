...
@endsection
@section('content')
<form class="ajax-input-form" action="{{ route('jo.update',$po->id) }}" method="POST" data-redirect="{{route('jo.index')}}" enctype="multipart/form-data">
@csrf
...
@section('scripts')
<script>
    let actionUrlTax = '{{route("jo.tax.calculation")}}';
</script>
...
function initializeAutocomplete1(selector, type) {
    $(selector).autocomplete({
        ...
        select: function(event, ui) {
            ...
            let actionUrl = "{{ route('jo.get.address') }}" 
    + '?id=' + itemId;
            fetch(actionUrl).then(response => {
                return response.json().then(data => {
...
let actionUrl = '{{ route("jo.item.row") }}'
    + '?count=' + rowsLength
