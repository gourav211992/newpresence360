...
return new class extends Migration
{
    ...
    {
        # Inspection Headers
        foreach(['erp_insp_headers', 'erp_insp_headers_history'] as $tbl) {
        ...
        # Inspection Item
        foreach(['erp_insp_details', 'erp_insp_details_history'] as $tbl) {
            Schema::create($tbl, function (Blueprint $table) use ($tbl) {
                $table->id();
                if (str_contains($tbl, 'history')) {
                    $table->unsignedBigInteger('source_id')->nullable();
                ...
                if (str_contains($tbl, 'history')) {
                    $table->foreign('header_id')->references('id')->on('erp_insp_headers_history')->onDelete('cascade');
                } else {
                    $table->foreign('header_id')->references('id')->on('erp_insp_headers')->onDelete('cascade');
                }
            });
        }

        # Inspection Item Attributes
        foreach(['erp_insp_attributes','erp_insp_attributes_history'] as $tbl) {
            Schema::create($tbl, function (Blueprint $table) use ($tbl) {
                $table->id();
                if (str_contains($tbl, 'history')) {
                    $table->unsignedBigInteger('source_id')->nullable();
        ...
        # Inspection Item Locations
        foreach(['erp_insp_item_locations','erp_insp_item_locations_history'] as $tbl) {
            Schema::create($tbl, function (Blueprint $table) use ($tbl) {
                $table->id();
                if (str_contains($tbl, 'history')) {
                    $table->unsignedBigInteger('source_id')->nullable();
        ...
        # Put Away TED
        foreach(['erp_insp_ted','erp_insp_ted_history'] as $tbl) {
            Schema::create($tbl, function (Blueprint $table) use ($tbl) {
                $table->id();
                if (str_contains($tbl, 'history')) {
                    $table->unsignedBigInteger('source_id')->nullable();
        ...
        Schema::create('erp_insp_media', function (Blueprint $table) use ($tbl) {
            $table->id();
            $table->morphs('model');
    ...
    {
        Schema::dropIfExists('erp_insp_media');
        Schema::dropIfExists('erp_insp_ted_history');
        Schema::dropIfExists('erp_insp_ted');
        Schema::dropIfExists('erp_insp_item_locations_history');
        Schema::dropIfExists('erp_insp_item_locations');
        Schema::dropIfExists('erp_insp_attributes_history');
        Schema::dropIfExists('erp_insp_attributes');
        Schema::dropIfExists('erp_insp_details_history');
        Schema::dropIfExists('erp_insp_details');
        Schema::dropIfExists('erp_insp_headers_history');
        Schema::dropIfExists('erp_insp_headers');
    }
};
