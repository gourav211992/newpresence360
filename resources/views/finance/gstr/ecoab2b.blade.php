    ...
    <tbody>
        @forelse ($gstrData as $key => $item)
            <tr class="trail-bal-tabl-none">
                ...
                <td>{{ $item->revised_doc_no ? $item->revised_doc_no : '-' }}</td>
                <td>{{ $item->revised_doc_date ? App\Helpers\GeneralHelper::dateFormat3($item->revised_doc_date) : '-' }}</td>
                <td>{{ $item->value_of_supplies_made }}</td> 
        ...
        @empty
            <tr>
                <td colspan="15" class="text-center text-danger">No record(s) found</td>
            </tr>
        @endforelse
    </tbody>
</table>
