        ...
        <div class="content-header row">
            <div class="content-header-left col-md-5 mb-2">
                <div class="row breadcrumbs-top">
                    <div class="col-12">
                        <h2 class="content-header-title float-start mb-0">Upload Vendor Data</h2>
            ...
            <div class="content-header-right text-sm-end col-md-7 mb-50 mb-sm-0">
                <div class="form-group breadcrumb-right">
                    <a href="{{ route('vendor.index') }}" class="btn btn-secondary btn-sm mb-50 mb-sm-0">
                        <i data-feather="arrow-left-circle"></i> Back
                    </a>
                    <a href="{{ asset('templates/Vendor_sample_template.xlsx') }}" class="btn btn-secondary btn-sm" id="download-template-btn" download>
        ...
        <div class="content-body">
            <section id="basic-datatable">
                <div class="row justify-content-center">
                    <div class="col-9">
                        <form class="importForm" method="POST" action="{{ route('vendors.import.post') }}" enctype="multipart/form-data">
                            @csrf
                            <div class="upload-item-masstrerdata">
                                ...
                                <div class="drapdroparea drapdroparea-small upload-btn-wrapper text-center" id="uploadSuccess" style="display: none;">
                                    ...
                                    Please proceed to process orders.</p>
                                    <div class="d-flex">
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('submit', '.importForm', function (e) {
            ...
            $.ajax({
                ...
                xhr: function() {
                    ...
                    xhr.upload.addEventListener('progress', function(e) {
                        if (e.lengthComputable) {
                            ...
                            console.log(`Loaded: ${e.loaded}, Total: ${e.total}, Percentage: ${percent}%`);
                ...
                success: function (res) {
                    ...
                    // Check response status and show alert
                    ...
                    // Populate tables with success and failure data for vendors
                    populateTable('#success-table-body', res.successful_vendors);
                    populateTable('#failed-table-body', res.failed_vendors);
                    
                    // Update counts
                    $('#success-count-badge').text(`Records Succeeded: ${res.successful_vendors.length}`);
                    $('#success-count').text(`(${res.successful_vendors.length})`);
                    $('#failed-count').text(`(${res.failed_vendors.length})`);
                    
                    $('#uploadProgress').hide();
                    $('.hide-this-section').show();

                    // Handle failed vendors and actions
                    if (res.failed_vendors.length > 0) {
                        $('.editbtnNew').show();  
                    } else {
                        $('.editbtnNew').hide(); 
                    }
                    ...
                    $('.exportBtn').on('click', function() {
                        ...
                        if (activeTab === '#Succeded') {
                            window.location.href = `/vendors/export-successful-vendors`; 
                        } else if (activeTab === '#Failed') {
                            window.location.href = `/vendors/export-failed-vendors`; 
                        }
                    });
                },
        ...
        function populateTable(tableBodySelector, vendors) {
            ...
            if (vendors.length > 0) {
                vendors.forEach((vendor, index) => {
                    const row = `
                        <tr>
                            <td>${index + 1}</td>
                            <td class="fw-bolder text-dark">${vendor.vendor_code}</td>
                            <td>${vendor.company_name}</td>
                            <td>${vendor.vendor_type}</td>
                            <td class="${vendor.status === 'success' ? 'text-success' : 'text-danger'}">
                                ${vendor.status === 'success' ? 'Success' : (vendor.status === 'failed' ? 'Failed' : vendor.remarks)}
                            </td>
                             <td class="${vendor.remarks ? 'text-danger' : 'text-danger'}">
                                ${vendor.remarks || 'N/A'}
                            </td>
                        </tr>
                    `;
                ...
                const noDataRow = `<tr><td colspan="6" class="text-center">No records found</td></tr>`;
