@extends('layouts.app')
@section('content')
    <form id="expenseEditForm" class="ajax-input-form" method="POST" action="{{ route('expense-adv.update', $mrn->id) }}" data-redirect="/expense-advice" enctype="multipart/form-data">
        @csrf
                    ...
                    <div class="row">
                        ...
                        <div class="content-header-right text-sm-end col-md-6 mb-50 mb-sm-0">
                            <div class="form-group breadcrumb-right">
                                ...
                                <a href="{{ route('expense-adv.generate-pdf', $mrn->id) }}" target="_blank" class="btn btn-dark btn-sm mb-50 mb-sm-0 waves-effect waves-float waves-light">
                                                                ...
                                                                <div class="genertedvariables genertedvariablesnone">
                                                                    ...
                                                                    <div class="mrnaddedd-prim billing_detail">
                                                                        @if($mrn->latestBillingAddress())
                                                                            ...
                                                                            <input type="hidden" name="billing_address" id="billing_address" value="{{$mrn->latestBillingAddress()->display_address}}">
                                                                        @else
                                                                            {{$mrn->billingAddress?->display_address}}
                                                                            <input type="hidden" name="billing_address" id="billing_address" value="{{$mrn->billingAddress?->display_address}}">
                                                                        @endif
                                                                    </div>
                                                                </div>
                                                            </div>
                                                    ...
                                                    <div class="col-md-12">
                                                        <div class="col-md-4">
                                                            <div class="mb-1">
                                                                ...
                                                                <input type="file" name="attachment[]" class="form-control" onchange = "addFiles(this,'main_expense_preview')" multiple>
                                                                <span class = "text-primary small">{{__("message.attachment_caption")}}</span>
                                                            </div>
                                                        </div>
                                                        @include('partials.document-preview',['documents' => $mrn->getDocuments(), 'document_status' => $mrn->document_status,'elementKey' => 'main_expense_preview'])
                                                    </div>
                                                    <div class="col-md-12">
                                                        <div class="mb-1">
    ...
    <script>
        ...
        function initializeAutocomplete1(selector, type) {
            $(selector).autocomplete({
                ...
                select: function(event, ui) {
                    ...
                    let actionUrl = "{{route('expense-adv.get.address')}}"+'?id='+itemId;
                    fetch(actionUrl).then(response => {
                        return response.json().then(data => {
                            if(data.status == 200) {
        ...
        $(document).on('click','#deleteBtn', (e) => {
            ...
            $('.form-check-input:checked').each(function(index, item) {
                ...
                let so_detail_id = Number($(tr).find('[name*="[so_detail_id]"]').val()) || 0;
                let expense_detail_id = Number($(tr).find('[name*="[expense_detail_id]"]').val()) || 0;
                if (so_detail_id > 0 && expense_detail_id > 0) {
                    soItemIds.push({ index: trIndex + 1, so_detail_id: so_detail_id });
                }
                if (po_detail_id > 0 && expense_detail_id > 0) {
        ...
        function getItemDetail(currentTr) {
            ...
            if (itemId) {
                ...
                let actionUrl = '{{route("expense-adv.get.itemdetail")}}' +
                    '?item_id=' + itemId +
