@foreach ($items as $key => $item)
    ...
    <tr id="row_{{ $rowCount }}" data-index="{{ $rowCount }}"
        ...
        <td class="poprod-decpt">
            <input type="text" readonly placeholder="Select" class="form-control mw-100 mb-25 ledgerselecct"
                value="{{ $item->{$type}->document_date }}" />
        </td>
        <td class="poprod-decpt">
            <input type="text" readonly placeholder="Select" class="form-control mw-100 mb-25 ledgerselecct"
                value="{{ $item->{$type}->document_number }}" />
        </td>
        <td class="poprod-decpt">
            <input type="text" readonly placeholder="Select"
                class="form-control mw-100 mb-25 ledgerselecct comp_item_code " value="{{ $item->item_code }}" />
        </td>
        <td>
            <input type="text" value="{{ $item?->item?->item_name }}" class="form-control mw-100 mb-25" readonly />
        </td>
        <td class="poprod-decpt">
            <button type="button" class="btn p-25 btn-sm btn-outline-secondary" data-row-count="{{ $rowCount }}"
        ...
        <td><input type="number" class="form-control mw-100 text-end" maxAmount="{{ $item->rejected_qty }}"
                value="{{ $item->rejected_qty }}" step="any">
        </td>
