        ...
        orders.forEach((norder, index) => {
            ...
            const row = `
                <tr>
                    ...
                    <td class="text-end balance-qty-cell">${norder.item.planned_balance_qty || '0.00'}</td>
                    <td class="text-end">${norder.avl_stock || '0.00'}</td>
                    <td class="text-end">${norder.item.planned_qty || '0.00'}</td>
                    <td class="text-end">${norder.item.picked_qty || '0.00'}</td>
            ...
            $checkbox.on('change', function () {
                if (this.checked) {
                    const savedQty = prevBalanceMap.get(norder.id) ?? (order ? order.planned_qty : null);
            ...
            if (order) {
                console.log('order', order.items, 'orderid', norder.id);
                const matchedItem = order.items.find(item => item.order_item_delivery_id === norder.id);
                if (matchedItem) {
                    $checkbox.prop('checked', true);
                    $checkbox.trigger('change');
                    const $input = createValidatedInput(norder, matchedItem.planned_qty);
                    $balanceQtyCell.html($input);
                }
            }
