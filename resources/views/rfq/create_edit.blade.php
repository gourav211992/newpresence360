...
<style>
    ...
    <!-- BEGIN: Content-->
    <form method="POST" data-completionFunction = "disableHeader" class="ajax-input-form sales_module_form material_issue" action = "{{route('rfq.store')}}" data-redirect="{{ $redirect_url }}" id = "sale_invoice_form" enctype='multipart/form-data'>
				...
				<div class="row">
					...
					<div class="content-header-right text-sm-end col-md-6 mb-50 mb-sm-0">
                        ...
                        @endif
                            @if (isset($order))
                                @if($buttons['print'])
                                    <a type="button" href="{{ route('rfq.generate-pdf', ['id' => $order -> id]) }}" target="_blank" class="btn btn-dark btn-sm mb-50 mb-sm-0 waves-effect waves-float waves-light dropdown-toggle" type="button"><i data-feather='printer'></i> Print</a>
                                @endif
                                @if($buttons['draft'])
                                ...
                                <div class="card-body customernewsection-form" id ="main_so_form">  
                                    <div class="row">
                                        ...
                                        <div class="col-md-8"> 
                                            ...
                                            @if (isset($order))
                                                <input type = "hidden" value = "{{$order -> id}}" name = "rfq_header_id"></input>
                                            @endif
                                ...
                                <div class="card-body customernewsection-form"> 
                                    <div class="bo_Sear-bottom mb-1 pb-10">
                                        <div class="row">
                                            <div class="col-md-10">
                                                <div class="newheader "> 
                                                    <h4 class="card-title text-theme">General Info</h4>
                                                </div>
                                            </div>
                                        </div> 
                                        <div class = "row">
                                            ...
                                            <div class="col-md-2">
                                                ...
                                                <input type="date" id="filter_due_date" name="due_date" class="form-control" autocomplete="off" value = "{{ isset($order->due_date) ? $order->due_date : null }}" placeholder="Enter Due Date" min="{{ isset($order) ? $order->document_date : Carbon\Carbon::now()->format('Y-m-d') }}">
                                            </div>
                                            <div class="col-md-2">
                                                <label for="filter_contact_person" class="form-label">Contact Person (Name)</label>
                                                <input type="text" id="filter_contact_person" name="contact_name" class="form-control" value = "{{ isset($order->contact_name) ? $order->contact_name : null }}" autocomplete="off" placeholder="Enter Contact Person Name">
                                            </div>
                                            <div class="col-md-2">
                                                <label for="filter_email" class="form-label">Email</label>
                                                <input type="email" id="filter_email" name="email" class="form-control" autocomplete="off" value = "{{ isset($order->contact_email) ? $order->contact_email : null }}" placeholder="Enter Email">
                                            </div>
                                            <div class="col-md-2">
                                                <label for="filter_phone_no" class="form-label">Phone No</label>
                                                <input type="text" id="filter_phone_no" name="phone" class="form-control" autocomplete="off" value = "{{ isset($order->contact_phone) ? $order->contact_phone : null }}" placeholder="Enter Phone Number">
                                            </div>
                                        </div>
                                ...
                                <div class="card-body customernewsection-form"> 
                                    ...
                                    <div class="border-bottom mb-2 pb-25">
                                        <div class="row">
                                            ...
                                            <div class="col-md-12">
                                                <textarea name="instructions" id="summernote" class="form-control" placeholder="Enter RFQ Instruction" >{{ isset($order->instructions) ? $order->instructions : '' }}</textarea>
                                                @error('rfq_instructions')
...
<div class="modal fade" id="approveModal" tabindex="-1" aria-labelledby="shareProjectTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form class="ajax-submit-2" method="POST" action="{{ route('document.approval.rfq') }}" data-redirect="{{ $redirect_url }}" enctype='multipart/form-data'>
                @csrf
...
<div class="modal fade" id="sendMail" tabindex="-1" aria-labelledby="shareProjectTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form class="ajax-submit-2" method="POST" action="{{ route('rfq.mail') }}" data-redirect="{{ $redirect_url }}" enctype='multipart/form-data'>
            @csrf
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                {{-- <form class="importForm" action="{{ route('rfq.import') }}" method="POST" enctype="multipart/form-data"> --}}
                    @csrf
                    <div class="mb-3">
...
initializeAutocompleteSearch('filter_item_name_code','filter_item_name_code_id','inventory_items','item_name');
    function resetStoreFields()
    {
        ...
        function addItemRow()
        {
            ...
            <tr id = "item_row_${newIndex}">
                ...
                <td class="numeric-alignment">
                    <input type="text" id="item_req_qty_${newIndex}" name="item_req_qty[]" class="form-control mw-100 text-end" onblur="setFormattedNumericValue(this);">
                </td>
                <td>
        ...
        function setValue(index)
        {
            ...
            selectedName = $("#rfq_item_id_" + index).val();
    ...
    function getOrders(type = "pi")
    {
        ...
        const apiUrl = "{{ route('rfq.get.items') }}";
    ...
    function processOrder()
    {
        ...
        const docType = $("#service_id_input").val();
        const apiUrl = "{{route('rfq.process.items')}}";
        // const apiUrl = "{{('rfq.process.items')}}";
        let docId = [];
        let soItemsId = [];
        let qties = [];
        let documentDetails = [];
        for (let index = 0; index < allCheckBoxes.length; index++) {
            if (allCheckBoxes[index].checked) {
                ...
                documentDetails.push({
                    ...
                    'quantity' : allCheckBoxes[index].getAttribute('rfq_balance_qty'),
                    ...
                    currentOrders.forEach((currentOrder) => {
                        currentOrder.items.forEach((item) => {
                            ...
                            <tr id="item_row_${itemRowIndex}" class="item_header_rows" onclick="onItemClick('${itemRowIndex}');">
                                ...
                                <td class="numeric-alignment">
                                    <input type="text" id="item_req_qty_${itemRowIndex}" value="${item.rfq_balance_qty}" oninput='changeItemQty(this,'${itemRowIndex}')' name="item_req_qty[]"
                                        max="${item.rfq_balance_qty}" class="form-control mw-100 text-end">
                                </td>
                                <td>
    ...
    items.forEach((item, index) => {
        tableBody += `
            <tr id="item_row_${index}" class="item_header_rows" onclick="onItemClick('${index}');" data-detail-id="${item.id}" data-id="${item.id}">
                <input type="hidden" id="rfq_item_id_${index}" name="rfq_item_id[]" value="${item.id}" ${item.is_editable ? '' : 'readonly'}>
                <td class="customernewsection-form">
                    <div class="form-check form-check-primary custom-checkbox">
...
function searchAjax()
{
    ...
    debounceTimer = setTimeout(function() {
        ...
        $.ajax({
            // url: "{{ ('rfq.search.items') }}",
            url: "{{ route('rfq.search.items') }}",
            type: "GET",
            dataType: "json",
            data: itemData,
            success: function (data) {
