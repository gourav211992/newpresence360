        ...
        <div class="content-header row">
            ...
            <div class="content-header-right text-sm-end col-md-3 mb-50 mb-sm-0">
                <div class="form-group breadcrumb-right">
                    ...
                    <a href="{{ route('customers.order-detail.csv',['customerCode' => $customer->customer_code]) }}{{ Request::getQueryString() ? '?' . Request::getQueryString() : '' }}" class="btn btn-primary btn-sm mb-50 mb-sm-0"><i data-feather="file-text"></i> Export to CSV</a>
                </div>
            </div>
        </div>
                            ...
                            <div class="table-responsive">
                                <table class="table myrequesttablecbox loanapplicationlist">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td class="fw-bolder text-dark">{{ $orderItem->order_date ? App\Helpers\GeneralHelper::dateFormat($orderItem->order_date) : '' }}</td>
                                            <td>{{ $orderItem->order_number }}</td>
                                            <td>{{ $orderItem->store_type }}</td>
                                            <td>{{ $orderItem->item_code }}</td>
                                            <td>{{ $orderItem->item_name }}</td>
                                            <td>{{ $orderItem->uom }}</td>
                                            <td>{{ $orderItem->delivery_date ? App\Helpers\GeneralHelper::dateFormat($orderItem->delivery_date) : '-' }}</td>
                                            <td>{{ $orderItem->total_order_value ? @$currencyMaster->symbol.''.$orderItem->total_order_value : '-' }}</td>
                                            <td>{{ $orderItem->order_quantity }}</td>
                                            <td>{{ $orderItem->delivered_quantity }}</td>
                                            <td>{{ $orderItem->order_quantity - $orderItem->delivered_quantity }}</td>
                                        </tr>
                                        @empty
