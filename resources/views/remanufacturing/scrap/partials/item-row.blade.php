...
<tr id="scavengingItemsTr_{{ $rowCount }}" data-index="{{ $rowCount }}">
    ...
    <td class="poprod-decpt">
        <input type="text"
               name="component_item_name[{{ $rowCount }}]"
               placeholder="Select"
               class="form-control mw-100 mb-25 ledgerselecct comp_item_code"
               value="{{ old("component_item_name.$rowCount", $item->item_code ?? '') }}"/>
    ...
    <td>
        <input type="text"
               name="components[{{ $rowCount }}][item_name]"
    ...
    {{-- Attributes Button --}}
    <td class="poprod-decpt attributeBtn"
        id="itemAttribute_{{ $rowCount }}"
        data-count="{{ $rowCount }}"
        attribute-array="{{ $item->attributes_json ?? '' }}">
        <button type="button"
    ...
    <td>
        <select class="form-select mw-100"
                name="components[{{ $rowCount }}][uom_id]">
            @if(!empty($uoms))
                @foreach($uoms as $uomId => $uomName)
                    <option value="{{ $uomId }}"
                        {{ isset($item->uom_id) && $item->uom_id == $uomId ? 'selected' : '' }}>
                        {{ $uomName }}
                    </option>
                @endforeach
            @endif
    ...
    <td>
        <input type="number" step="any"
               class="form-control text-end mw-100"
