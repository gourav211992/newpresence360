                ...
                <div class="content-header-right text-sm-end col-md-7 mb-50 mb-sm-0">
                    <div class="form-group breadcrumb-right">
                        ...
                        <a class="btn btn-primary btn-sm mb-50 mb-sm-0"
                            href="{{ route('finance.fixed-asset.split.create') }}"><i data-feather="plus-circle"></i> Add
                            New</a>
                    </div>
                </div>
            </div>
                                        ...
                                        <tbody>
                                            @foreach ($data as $key => $d)
                                                <tr>
                                                    ...
                                                    <td class="text-nowrap">
                                                        @php
                                                            $assets = collect(json_decode($d->sub_assets));

                                                            // Step 1: Get unique ledger IDs
                                                            $uniqueLedgerIds = $assets->pluck('ledger')->unique()??[];

                                                            // Step 2: Fetch ledger names from the database (or from an array if already loaded)
                                                            $ledgerNames = \App\Models\Ledger::whereIn(
                                                                'id',
                                                                $uniqueLedgerIds,
                                                            )->pluck('name');

                                                            // Step 3: Convert to comma-separated string
                                                            $ledgersname = $ledgerNames
                                                                ->unique()
                                                                ->implode(', ');
                                                        @endphp
                                                        {{$ledgersname??""}}
                                                                ...
                                                                <div class="dropdown-menu dropdown-menu-end">
                                                                    @if ($d->document_status == 'draft')
                                                                        <a class="dropdown-item"
                                                                            href="{{ route('finance.fixed-asset.split.edit', $d->id) }}">
                                                                            <i data-feather="edit" class="me-50"></i>
                                                                            <span>View</span>
                                                                        </a>
                                                                    @else
                                                                        <a class="dropdown-item"
                                                                            href="{{ route('finance.fixed-asset.split.show', $d->id) }}">
                                                                            <i data-feather="edit" class="me-50"></i>
                                                                            <span>View</span>
                                                                        </a>
                                                                    @endif
