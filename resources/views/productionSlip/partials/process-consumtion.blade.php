@foreach ($consumptions as $pwoBomConsIndex => $consumption)
    <tr id = "item_row_{{$pwoBomConsIndex}}" class = "item_header_rows" data-detail-id = "{{$consumption -> id}}" data-id = "{{$consumption -> id}}">
        ...
        <td>
            <input type="text" id = "item_qty_{{$pwoBomConsIndex}}" data-bom-qty="{{$consumption->bom_qty}}" data-mo-product-id="{{$consumption->mo_product_id}}" value = "{{$consumption->required_qty}}" name = "cons[{{$pwoBomConsIndex}}][item_qty]"  class="form-control mw-100 text-end" readonly/>
        ...
        <input type="hidden" id="so_item_id_{{$pwoBomConsIndex}}" name="so_item_id[{{$pwoBomConsIndex}}]"  value="{{$consumption?->so_item_id}}">
