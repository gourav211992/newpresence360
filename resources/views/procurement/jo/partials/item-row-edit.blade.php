@foreach($po->joProducts as $key => $po_item)
@php
   $rowCount = $key + 1;
@endphp
<tr id="row_{{$rowCount}}" data-index="{{$rowCount}}" @if($rowCount < 2 ) class="trselected" @endif>
 ...
 <td class="poprod-decpt"> 
    <input type="text" name="component_item_name[{{$rowCount}}]" placeholder="Select" class="form-control mw-100 mb-25 ledgerselecct comp_item_code " value="{{$po_item->item_code}}" />
...
<td>
   <div class="d-flex">
    @foreach($po_item->productDelivery as $itemDeli_key => $itemDelivery)
...
<input type="hidden" name="components[{{$rowCount}}][jo_product_id]" value="{{$po_item?->id}}">
<input type="hidden" name="components[{{$rowCount}}][pwo_id]" value="{{$po_item?->pwoSoMapping?->pwo_id}}">
<input type="hidden" name="components[{{$rowCount}}][pwo_so_mapping_id]" value="{{$po_item?->pwoSoMapping?->id}}">
</tr>
@endforeach