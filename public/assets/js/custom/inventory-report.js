...
function updateTable(inventory_reports = [], columnVisibility = []) {
    ...
    inventory_reports.forEach((report, index) => {
        ...
        const confirmedStockCost = (confirmedStock && !isNaN(confirmedStockValue) && confirmedStock > 0)
            ? (confirmedStockValue / confirmedStock).toFixed(2)
            : '0.00';
        const unconfirmedStockCost = (unconfirmedStock && !isNaN(unconfirmedStockValue) && unconfirmedStock > 0)
        ...
        const cells = [
            ...
            `<td class='text-end'>${report?.confirmed_stock_day4_days ?? 0.00}</td>`, // 10 Days Ago
            `<td class='text-end'>${report?.confirmed_stock_day5_days ?? 0.00}</td>`, // 15 Days Ago
            `<td class='text-end'>${report?.confirmed_stock__more_than_day5_days ?? '0.00'}</td>`, // 20 Days Ago
        ];
