...
@php
   $rowCount = $key + 1;
   $piMappingData = [];
   if($po_item?->pi_item_mappings?->count()) {
    $piMappingData['item_id'] = $po_item->item_id;
    $piMappingData['uom_id'] = $po_item->uom_id;
    $itemAttributes = [];
    foreach ($po_item->attributes as $attribute) {
        $itemAttributes[] = "{$attribute->item_attribute_id}:{$attribute->attribute_value}";
    }
    $piMappingData['attributes'] = implode(',', $itemAttributes);
    $piMappingData['total_qty'] = $po_item->order_qty;
    $piMappingData['pi_item_ids'] = $po_item->pi_item_mappings()->pluck('pi_item_id')->implode(',');
   }
@endphp
<tr @if(count($piMappingData)) data-group-item="{{json_encode($piMappingData)}}" @endif id="row_{{$rowCount}}" data-index="{{$rowCount}}" @if($rowCount < 2 ) class="trselected" @endif>
