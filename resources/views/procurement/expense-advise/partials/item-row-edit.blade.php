@foreach($mrn->items as $key => $item)
   ...
   <tr id="row_{{$rowCount}}" data-index="{{$rowCount}}" @if($rowCount < 2 ) class="trselected" @endif>
      <input type="hidden" name="components[{{$rowCount}}][expense_header_id]" value="{{$item->expense_header_id}}">
      <input type="hidden" name="components[{{$rowCount}}][expense_detail_id]" value="{{$item->id}}">
      <input type="hidden" name="components[{{$rowCount}}][po_detail_id]" value="{{$item->purchase_order_item_id}}">
      <!-- <input type="hidden" name="components[{{$rowCount}}][so_detail_id]" value="{{$item->sale_order_item_id}}"> -->
