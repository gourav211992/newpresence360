...
$(document).on('change',"[name*='order_qty']",(e) => {
    ...
    let detailId = $(e.target).closest('tr').find("[name*='inspection_item_id']").val() || '';
    ...
    if(mrnDetailId){
        ...
        '&detailId='+detailId+
        '&inspection_qty='+receiptQuantity.val();
        
        fetch(actionUrl).then(response => {
            return response.json().then(data => {
                console.log('data.data', data.data);
                let aq = parseFloat(receiptQuantity.val());
                ...
                if(data.data.error_message) {
                    ...
                    receiptQuantity.val(data.data.quantity);
                    ...
                    receiptQuantity.val(aq.toFixed(2));
...
/*accepted_qty on change*/
$(document).on('change',"[name*='accepted_qty']",(e) => {
    let tr = e.target.closest('tr');
    let qty = e.target;
    let qtyType = 'accepted';
    let dataIndex = $(e.target).closest('tr').attr('data-index');
    let itemId = $(e.target).closest('tr').find('[name*=item_id]').val();
    let receiptQuantity = $(e.target).closest('tr').find("[name*='order_qty']");
    let acceptedQuantity = $(e.target).closest('tr').find("[name*='accepted_qty']");
    ...
    if(mrnDetailId){
        ...
        '&detailId='+detailId+
        '&inspection_qty='+receiptQuantity.val()+
        '&accepted_qty='+acceptedQuantity.val();
...
function ledgerStock(currentTr, itemId, selectedAttr, itemStoreData){
    if (itemId) {
        let actionUrl = '/inspection/get-stock-detail?item_id='+itemId+'&selectedAttr='+JSON.stringify(selectedAttr)+'&itemStoreData='+JSON.stringify(itemStoreData);
        fetch(actionUrl).then(response => {
            return response.json().then(data => {
                if(data.status == 200) {
