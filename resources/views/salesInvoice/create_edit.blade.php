                                        ...
                                        <div class="row">
                                            ...
                                            @endif
                                                    ...
                                                    <div class="row align-items-center mb-1 lease-hidden">
                                                        ...
                                                        <div class="col-md-5">  
                                                            <select class="form-select disable_on_edit" name = "store_id" id = "store_id_input" oninput = "onHeaderLocationChange(this);">
                                                                @foreach ($stores as $store)
                                                                    <option display-address = "{{$store -> address ?-> display_address}}" value = "{{$store -> id}}" {{isset($order) ? ($order -> store_id == $store -> id ? 'selected' : '') : ''}} data-name = "{{$store -> store_name}}">{{$store -> store_name}}</option> 
                                                                @endforeach
                                                            </select>
                                                        </div>
                                                    </div>
                                                            ...
                                                            <thead>
                                                                ...
                                                                @if (isset($order))
                                                                    ...
                                                                    @foreach ($order -> items as $orderItemIndex => $orderItem)
                                                                        ...
                                                                        <input id = "so_item_id_{{$orderItemIndex}}" type = 'hidden' name = "so_item_id[]" value = "{{$orderItem -> id}}" {{$orderItem -> is_editable ? '' : 'readonly'}}>
                                                                        ...
                                                                        <td>
                                                                            <select class="form-select" name = "uom_id[]" id = "uom_dropdown_{{$orderItemIndex}}" onchange = "getStoresData({{$orderItemIndex}}, '', true)">
                                                                       ...
                                                                       <td><input type="text" id = "item_rate_{{$orderItemIndex}}" name = "item_rate[]" {{$docType == 'dnote' ? 'readonly' : ''}} oninput = "changeItemRate(this, '{{$orderItemIndex}}');" value = "{{$orderItem -> rate}}" class="form-control mw-100 text-end" onblur = "setFormattedNumericValue(this);" /></td> 
                                                                        ...
                                                                        <input type="hidden" id = "item_total_{{$orderItemIndex}}" value = "{{($orderItem -> order_qty * $orderItem -> rate) - $orderItem -> item_discount_amount - $orderItem -> header_discount_amount + ($orderItem -> tax_amount)}}" disabled class="form-control mw-100 text-end item_totals_input" />
                                                                         <td>
                                                                             <div class="d-flex">
                                                                                 <div style = "display:none;" class="me-50 cursor-pointer item_store_locations" data-bs-toggle="modal" data-bs-target="#location" onclick = "openStoreLocationModal('{{$orderItemIndex}}')" data-stores = '[]' id = "data_stores_{{$orderItemIndex}}">    <span data-bs-toggle="tooltip" data-bs-placement="top" title="Store Location" class="text-primary"><i data-feather="map-pin"></i></span></div>
                                                                    ...
                                                                    <td id = "item_details_td" colspan="8" rowspan="10">
                                                                        <table class="table border">
                                                                            ...
                                                                            <tr id = "current_item_cust_details_header"> 
                                                                                <td class="poprod-decpt">
                                                                                    <div id ="current_item_cust_details">
...
<div class="modal fade" id="podModal" tabindex="-1" aria-labelledby="shareProjectTitle" aria-hidden="true">
   <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <form class="ajax-submit-2" method="POST" action="{{ route('sale.invoice.pod') }}" data-redirect="{{ $redirect_url }}" enctype='multipart/form-data'>
          @csrf
          <input type="hidden" name="action_type" id="action_type">
          <input type="hidden" name="id" value="{{isset($order) ? $order -> id : ''}}">
         <div class="modal-header">
         ...
         <div class="modal-footer justify-content-center">  
            <button type="reset" class="btn btn-outline-secondary me-1" onclick = "closeModal('podModal');">Cancel</button> 
            <button type="submit" class="btn btn-primary">Submit</button>
         </div>
       </form>
      </div>
...
<div class="modal fade" id="sendMail" tabindex="-1" aria-labelledby="shareProjectTitle" aria-hidden="true">
   <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <form class="ajax-submit-2" method="POST" action="{{ route('sale.invoice.eInvoiceMail') }}" data-redirect="{{ $redirect_url }}" enctype='multipart/form-data'>
          @csrf
          <input type="hidden" name="action_type" id="action_type">
          <input type="hidden" name="id" value="{{isset($order) ? $order -> id : ''}}">
         <div class="modal-header">
            ...
            <div class="row mt-1">
                ...
                <div class="col-md-12">
                    <div class="mb-1">
                        ...
                        <input type="text" id='cust_mail' name="email_to" class="form-control cannot_disable">
                    </div>
                </div>
                <div class="col-md-12">
                    <div class="mb-1">
...
<script>
        ...
        function itemOnChange(selectedElementId, index, routeUrl) // Retrieve element and set item attiributes
        {
            ...
            if (selectedElement && ItemIdDocument) {
                ...
                fetch(apiUrl, {
                    ...
                    onItemClick(index);
                    setLocationStoreOnItem(response.item.id, index);
        ...
        function onItemClick(itemRowId)
        {
            ...
            let locationsHTML = ``;
            var showLocationIcon = false;
            if (locations && locations.length > 0) {
                locations.forEach((loc) => {
                    if (loc.qty > 0) {
                        if (loc.bin_code || loc.shelf_code || loc.rack_code) {
                            showLocationIcon = true;
                            locationsHTML += `<span class="badge rounded-pill badge-light-primary"><span class="badge rounded-pill badge-light-primary"><strong>Rack</strong>: ${loc.rack_code}</span><span class="badge rounded-pill badge-light-primary"><strong>Shelf</strong>: ${loc.shelf_code}</span><span class="badge rounded-pill badge-light-primary"><strong>Bin</strong>: ${loc.bin_code}</span><span class="badge rounded-pill badge-light-primary"><strong>Qty</strong>: ${loc.qty}</span><br/><br/>`;
                        }
                    }
                });
            ...
            if (showLocationIcon) {
                document.getElementById('data_stores_' + itemRowId).style.display = "";
            } else {
                document.getElementById('data_stores_' + itemRowId).style.display = "none";
            }
                    ...
                    if (itemAttributes && itemAttributes.length > 0) {
                        ...
                        $.ajax({
                            ...
                            data: {
                                ...
                                so_item_id : $("so_item_id_" + itemRowId).val(),
                                service_alias : 'dnote',
                            ...
                            success: function(data) {
                                ...
                                const customerItemSectionHeader = document.getElementById('current_item_cust_details_header');
                                const customerItemSection = document.getElementById('current_item_cust_details');
                                if (data && data?.customer_item_details && data?.customer_item_details?.customer_item_id) {
                                    customerItemSectionHeader.style.removeProperty('display');
                                    customerItemSection.innerHTML = `
                                    <span class="badge rounded-pill badge-light-primary"><strong>Customer Item Code</strong>: <span>${ data?.customer_item_details?.customer_item_code}</span></span>
                                    <span class="badge rounded-pill badge-light-primary"><strong>Customer Item Name</strong>: <span>${ data?.customer_item_details?.customer_item_name}</span></span>
                                    `;
                                } else {
                                    customerItemSectionHeader.style.display = "none";
                                }
                                ...
                                if (docType === 'dnote' || docType === 'sinvdnote') {
                                    ...
                                    `;
                                    if (data?.stocks?.reservedStockAltUom) {
                                        document.getElementById('current_item_stocks').innerHTML += `
                                        <span class="badge rounded-pill badge-light-primary"><strong>Reserved Stock</strong>: <span id = "item_category">${data?.stocks?.reservedStockAltUom}</span></span>
                                        `
                                    }
                                    var inputQtyBox = document.getElementById('item_qty_' + itemRowId);
                                    inputQtyBox.setAttribute('max-stock',data.stocks.confirmedStockAltUom + data.stocks.reservedStockAltUom);
                                    ...
                                    //     if (data.inv_qty > data.stocks.confirmedStocks) {
                                    //         inputQtyBox.value = parseFloat(data.stocks.confirmedStockAltUom).toFixed(2);
                                    //         Swal.fire({
                                    //             title: 'Error!',
                                    ...
                                    // if (check) {
                                    //     var value = parseFloat(inputQtyBox.value);
                                    //     if (data.stocks.confirmedStocks <= 0) {
                                    //         inputQtyBox.value = 0;
                                ...
                                if (data?.lot_details && data?.lot_details.length > 0) {
