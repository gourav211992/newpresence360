@extends('layouts.app')
@section('content')
    <form class="ajax-input-form" method="POST" action="{{ route('purchase-bill.update', $mrn->id) }}" data-redirect="/purchase-bills" enctype="multipart/form-data">
        @csrf
                                    ...
                                    <div class="card-body customernewsection-form">
                                        ...
                                        <div class="row">
                                            <div class="col-md-8">
                                                ...
                                                <div class="row align-items-center mb-1">
                                                    <div class="col-md-3"> 
                                                        <label class="form-label">Pb No <span class="text-danger">*</span></label>  
                                                    </div>
                                                    <div class="col-md-5"> 
                                                ...
                                                <div class="row align-items-center mb-1">
                                                    <div class="col-md-3"> 
                                                        <label class="form-label">Pb Date <span class="text-danger">*</span></label>  
                                                    </div>
                                                    <div class="col-md-5"> 
...
@section('scripts')
    ...
    <script>
        ...
        {
            let actionUrl = '{{route("purchase-bill.tax.calculation")}}';
        ...
        $(document).on('click', '.addDiscountItemRow', (e) => {
            ...
            if(disName && (disPerc || disAmount)) {
                ...
                let actionUrl = '{{route("purchase-bill.item.discount.row")}}'+'?tbl_row_count='+tblRowCount+'&row_count='+rowCount+'&dis_name='+disName+'&dis_percentage='+disPerc+'&dis_amount='+disAmount;
                fetch(actionUrl).then(response => {
                    return response.json().then(data => {
        ...
        $(document).on('click','#deleteConfirm', (e) => {
            ...
            let actionUrl = '{{route("purchase-bill.comp.delete")}}'+'?ids='+dataIds;
            fetch(actionUrl, {
                method: 'DELETE',
                headers: {
        ...
        $(document).on('click','#deleteItemDiscConfirm', (e) => {
            ...
            let actionUrl = '{{route("purchase-bill.remove.item.dis")}}'+'?id='+dataId;
            fetch(actionUrl, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
        ...
        $(document).on('click','#deleteHeaderDiscConfirm', (e) => {
            let dataId = e.target.getAttribute('data-id');
            let actionUrl = '{{route("purchase-bill.remove.header.dis")}}'+'?id='+dataId;
            fetch(actionUrl, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
        ...
        $(document).on('click','#deleteHeaderExpConfirm', (e) => {
            let dataId = e.target.getAttribute('data-id');
            let actionUrl = '{{route("purchase-bill.remove.header.exp")}}'+'?id='+dataId;
            fetch(actionUrl, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
        ...
        $(document).on('click', '#amendmentSubmit', (e) => {
            let actionUrl = "{{ route('purchase-bill.amendment.submit', $mrn->id) }}";
            fetch(actionUrl).then(response => {
                return response.json().then(data => {
                    if (data.status == 200) {
                        Swal.fire({
