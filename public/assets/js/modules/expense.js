...
function setTableCalculation() {
    ...
    let grandTotal = 0;
    // let taxValue = $("#tax_required").val().trim().toLowerCase();
    // let isTax = taxValue.includes('yes');
    let isTax = $("#tax_required").val().includes('yes') ? true : false;
    ...
    Promise.all(taxPromises).then(() => {
        ...
        $("#itemTable [id*='row_']").each(function (index, item4) {
            ...
            console.log('isTax', isTax);
            if (isTax) {
                console.log('isTax', $(item4).find("[name*='[t_perc]']").length, item4);
