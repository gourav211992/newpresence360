...
@section('content')
    ...
    @endphp
    <form class="ajax-input-form" method="POST" action="{{ route('inspection.store') }}" data-redirect="/{{$routeRedirect}}" enctype="multipart/form-data">
            ...
            fetch(actionUrl).then(response => {
                return response.json().then(data => {
                    if(data.status == 200) {
                        ...
                        console.log('mrnHeader', mrnHeader);
                        $('.gate_entry_no').val(mrnHeader.gate_entry_no);
                        $('.gate_entry_date').val(mrnHeader.gate_entry_date);
                        $('.eway_bill_no').val(mrnHeader.eway_bill_no);
                        $('.consignment_no').val(mrnHeader.consignment_no);
                        $('.supplier_invoice_number').val(mrnHeader.supplier_invoice_no);
                        $('.supplier_invoice_date').val(mrnHeader.supplier_invoice_date);
                        $('.transporter_name').val(mrnHeader.transporter_name);
                        $('.vehicle_no').val(mrnHeader.vehicle_no);
