...
function setTableCalculation(edit = false) {
    ...
    if ($(".display_summary_discount_row").find("[name*='[d_perc]']").length && totalAmountAfterItemDis) {
        $(".display_summary_discount_row").find("[name*='[d_perc]']").each(function(index, eachItem) {
            ...
            console.log('disHeaderAmnt', disHeaderAmnt);
...
function ledgerStock(currentTr, itemId, selectedAttr, itemStoreData){
    if (itemId) {
        let actionUrl = '/gate-entries/get-stock-detail?item_id='+itemId+'&selectedAttr='+JSON.stringify(selectedAttr)+'&itemStoreData='+JSON.stringify(itemStoreData);
        fetch(actionUrl).then(response => {
            return response.json().then(data => {
                if(data.status == 200) {
...
function getSelectedItemAmount(poItemIds, poIds, tedId, callback) {
    ...
    .then(data => {
        if (data.status === 200) {
            console.log('Selected Item Amount:', data.data.poItemValue);
