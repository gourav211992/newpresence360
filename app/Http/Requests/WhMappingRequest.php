...
class WhMappingRequest extends FormRequest
{
    ...
    public function messages()
    {
        return [
            ...
            'details.*.name.max' => 'Name may not be greater than 255 characters.',
    ...
    public function withValidator($validator)
    {
        ...
        foreach ($details as $index => $detail) {
            ...
            foreach ($parentIds as $parentId) {
                $heirarchyName = $detail['name'] . '-' . optional(WhDetail::find($parentId))->name;
                ...
                if ($duplicate) {
                    $validator->errors()->add("details.$index.name", "Duplicate hierarchy: '$heirarchyName'");
                }
            }
        }
    }
