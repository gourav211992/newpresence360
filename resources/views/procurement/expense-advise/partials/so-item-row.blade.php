@foreach($soItems as $key => $item)
    ...
    <tr id="row_{{$rowCount}}" data-index="{{$rowCount}}" @if($rowCount < 2 ) class="trselected" @endif>
        ...
        <td>
            <input type="hidden" class="form-control mw-100 expense_qty" name="components[{{$rowCount}}][expense_qty]" value="{{($item->order_qty - $item->expense_advise_qty)}}" />
            <input type="number" class="form-control mw-100 accepted_qty text-end checkNegativeVal" name="components[{{$rowCount}}][accepted_qty]" value="{{($item->order_qty - $item->expense_advise_qty)}}" step="any"/>
        </td>
        <td>
        ...
        <td>
            <input type="number" name="components[{{$rowCount}}][basic_value]" value="{{($item->accepted_qty - $item->purchase_bill_qty)*$item->rate}}"  class="form-control text-end mw-100 basic_value checkNegativeVal" readonly step="any"/>
        </td>
        <td>
            <div class="position-relative d-flex align-items-center">
                @foreach($item->itemDiscount as $itemDis_key => $itemDiscount)
                    ...
                    <input type="hidden" value="{{$itemDiscount->ted_id}}" name="components[{{$rowCount}}][discounts][{{$itemDis_key+1}}][dis_ted_id]">
