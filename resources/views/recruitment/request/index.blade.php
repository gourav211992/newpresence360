    ...
    <div class="content-wrapper container-xxl p-0">
            ...
            <div class="content-header-left col-md-6 mb-2">
                ...
                <div class="form-group breadcrumb-right"> 
                        ...
                        <a href="{{ route('recruitment.requests.create') }}" class="btn btn-primary  btn-sm mb-50 mb-sm-0"><i data-feather="plus-square" ></i> Add Request</a>   
                </div>
            </div>
        </div>
                                            ...
                                            <tbody>
                                                @forelse ($requests as $request)
                                                    <tr>
                                                        ...
                                                        <td class="text-nowrap">{{ $request->created_at ? App\Helpers\CommonHelper::dateFormat($request->created_at) : '' }}</td>
                                                        ...
                                                        <td class="">
                                                            <div class="d-flex flex-row">
                                                                ...
                                                                <div class="my-auto">
                                                                    <h6 class="mb-0 fw-bolder text-dark hr-dashemplname">{{ $request->creator_name }}</h6>
                                                                </div>
                                                            </div>
                                                        ...
                                                        <td>{{ $request->expected_doj ? App\Helpers\CommonHelper::dateFormat($request->expected_doj) : '' }}</td>
                                                        <td>
                                                            @php
                                                                if(in_array($request->status, ['approved-forward','final-approved','active'])){
                                                                    $className = "badge-light-success";
                                                                }elseif(in_array($request->status, ['pending','onhold','rejected','cancelled','revoke','inactive'])){
                                                                    $className = "badge-light-danger";
                                                                }else{
                                                                    $className = "badge-light-warning";
                                                                }
                                                            @endphp
                                                            <span class="badge rounded-pill {{ $className }} badgeborder-radius">
                                                            ...
                                                            <div class="dropdown">
                                                                ...
                                                                <div class="dropdown-menu dropdown-menu-end">
                                                                    <a class="dropdown-item" href="{{ route('recruitment.requests.show',['id' => $request->id]) }}">
                                                                        <i data-feather="eye" class="me-50"></i>
                                                                        <span>View Detail</span>
                                                                    </a> 

                                                                    @if($request->status != 'final-approved' && $user->id == $request->created_by)
                                                                        <a class="dropdown-item" href="{{ route('recruitment.requests.edit',['id' => $request->id]) }}">
                                                                    ...
                                                                    @if($request->status == App\Helpers\CommonHelper::PENDING && $user->id == $request->created_by)
                                                                        <a class="dropdown-item" href="javascript:;" data-bs-target="#status-modal" data-bs-toggle="modal" data-status="{{ App\Helpers\CommonHelper::REVOKED }}" data-title="Revoked Request" data-id="{{ $request->id }}">
                                                                            <i data-feather="trash-2" class="me-50"></i>
                                                                            <span>Revoke</span>
                                                                        </a> 
                                                                    @endif

                                                                    @if(in_array($request->status, [App\Helpers\CommonHelper::PENDING, App\Helpers\CommonHelper::APPROVED_FORWARD]) && $request->approval_authority == $user->id && $user->manager_id)
                                                                        <a class="dropdown-item" href="javascript:;" data-bs-target="#status-modal" data-bs-toggle="modal" data-status="{{ App\Helpers\CommonHelper::APPROVED_FORWARD }}" data-title="Approved Request" data-id="{{ $request->id }}">
                                                                    ...
                                                                    @if(in_array($request->status, [App\Helpers\CommonHelper::PENDING, App\Helpers\CommonHelper::APPROVED_FORWARD]) && $request->approval_authority == $user->id && !$user->manager_id)
                                                                        <a class="dropdown-item" href="javascript:;" data-bs-target="#status-modal" data-bs-toggle="modal" data-status="{{ App\Helpers\CommonHelper::FINAL_APPROVED }}" data-title="Approved Request" data-id="{{ $request->id }}">
                                                                            <i data-feather="check-circle" class="me-50"></i>
                                                                            <span>Final Approved</span>
                                                                        </a> 
                                                                        <a class="dropdown-item" href="javascript:;" data-bs-target="#status-modal" data-bs-toggle="modal" data-status="{{ App\Helpers\CommonHelper::REJECTED }}" data-title="Reject Request" data-id="{{ $request->id }}">
                                                ...
                                                @empty
                                                    <tr><td class="text-danger text-center" colspan="13">No record(s) found.</td></tr>
                                                @endforelse
...
<div class="modal fade" id="status-modal" tabindex="-1" aria-labelledby="statusModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form role="post-data" method="POST" id="status-form" action="" redirect="{{route('recruitment.requests') }}">
