        ...
        success: function (data) {
            if (data.status === 200 && data.is_exist === 1) {
                ...
                mappings.forEach((mapping, index) => {
                    ...
                    const structures = Array.isArray(mapping.structures) ? mapping.structures : [];
                    ...
                    structures.forEach((structure, levelIndex) => {
                        ...
                        const levelOptions = (structure.options || []).map(opt =>
                        ...
                        structureHtml += `
                            <div class="form-group me-2"  style="min-width: 220px; display: flex; align-items: center;">
                                <label class="form-label" style="font-weight: bold; padding-right:10px;  margin-bottom: 4px;">${structure.name}</label>  
                                <select class="form-select select2 child-dropdown" 
                                        data-level-index="${levelIndex}" 
                                        data-level-name="${levelName}" 
                                        name="details[${index + 1}][${levelName}][]"
                                        multiple>
                                    ${levelOptions}
                                </select>
                            </div>`;
                    ...
                    let rowHtml = `
                        <tr class="item-row">
                            <td class="customernewsection-form">
                                ...
                                <input type="hidden" name="details[${index + 1}][detail_id]" value="${detail_id}">
                            </td>
                            <td>
                                <div class="d-flex align-items-center">
                                    <div class="form-group me-2"  style="min-width: 220px; display: flex; align-items: center;">
                                        <select class="form-select select2 category-select" name="details[${index + 1}][category_id][]" multiple>
                                            ${categoryOptions}
                                        </select>
                                    </div>
                                </div>
                            ...
                            <td>
                                <div class="d-flex align-items-center">
                                    <div class="form-group me-2"  style="min-width: 220px; display: flex; align-items: center;">
                                        <select class="form-select select2 sub-category-select" name="details[${index + 1}][sub_category_id][]" multiple>
                                            ${subCategoryOptions}
                                        </select>
                                    </div>
                                </div>
                            ...
                            <td>
                                <div class="d-flex align-items-center">
                                    <div class="form-group me-2"  style="min-width: 220px; display: flex; align-items: center;">
                                        <select class="form-select select2 item-select" name="details[${index + 1}][item_id][]" multiple>
                                            ${itemOptions}
                                        </select>
                                    </div>
                                </div>
...
$(document).on('click', '.addNewItemBtn', (e) => {
    ...
    var url = "/warehouse-item-mappings/details?"+"store_id=" + storeId +"&sub_store_id=" + subStoreId;
    $.ajax({
        type: "GET",
        url: url,
        async: false,
        success: function (data) {
            if (data.status == 200) {
                ...
                structures.forEach((level, index) => {
                    let selectOptions = (level.storage_point_details || []).map(option => 
    ...
    $(".mrntableselectexcel tr").each(function (levelCounter) {
        ...
        $(this).find("select.category-select").attr("name", `details[${levelCounter + 1}][category_id][]`);
        $(this).find("select.sub-category-select").attr("name", `details[${levelCounter + 1}][sub_category_id][]`);
        $(this).find("select.item-select").attr("name", `details[${levelCounter + 1}][item_id][]`);
        
        // For each row, find all child-dropdowns
        $(this).find("select.child-dropdown").each(function (levelIndex) {
            const levelName = $(this).data("level-name") || `level_${levelIndex}`;
            $(this).attr("name", `details[${levelCounter + 1}][${levelName}][]`);
        });
    });
                ...
                if (response.status == 200) {
                    ...
                    response.data.forEach(function (subCat) {
                        if (!existingSubCatIds.has(subCat.id.toString())) {
                            subCatSelect.append(`<option value="${subCat.id}">${subCat.name}</option>`);
                        }
                    });
...
$(document).on('change', '.child-dropdown', function () {
    ...
    let $nextDropdown = $row.find(`select.child-dropdown[data-level-index="${currentLevelIndex + 1}"]`);
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status === 200) {
                    ...
                    let retained = existingSelected.filter(val => validValues.includes(val));
                    ...
                    success: function (response) {
                        ...
                        $(".mrntableselectexcel tr").each(function (index) {
                            $(this).find("input[name^='details']").each(function () {
                                ...
                                if (nameAttr) {
                                    let updatedName = nameAttr.replace(/\[\d+\]/, `[${index + 1}]`);
...
$(document).on('change focus blur', '.category-select, .sub-category-select, .item-select', function () {
    updateUniqueSelectOptions();
});
