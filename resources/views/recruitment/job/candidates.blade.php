...
<div class="app-content content ">
    ...
    <div class="content-wrapper container-xxl p-0">
        <form class="form" role="post-data" method="POST" action="{{ route('recruitment.jobs.assign-candidate',['id' => $job->id]) }}" redirect="{{ route('recruitment.jobs') }}" autocomplete="off">
                ...
                <div class="content-header-right text-sm-end col-md-6 ">
                    <div class="form-group breadcrumb-right">
                        ...
                        <button class="btn btn-primary btn-sm mb-50 mb-sm-0" data-request="ajax-submit" data-target="[role=post-data]"><i
                                        data-feather="check-circle"></i> Assign</button>
                    </div>
                </div>
            </div>
                                    ...
                                    <table class="datatables-basic table table-striped">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td class="pe-0">
                                                    <div class="form-check form-check-inline">
                                                        <input class="form-check-input" type="checkbox" name="candidate_ids[]" value="{{ $candidate->id }}" {{ in_array($candidate->id, $assignedCandidateIds) ? 'checked' : '' }}>
                                                    </div>
                                                </td>
                                                <td>
                                                    <div class="cand-inforecsec">
                                                        <div class="">
                                                            ...
                                                            <h5>{{ $candidate->mobile_no }} <a>{{ $candidate->email }}</a>
                                                            </h5>
                                                        </div>
                                                ...
                                                <td><a href="{{ url('/').'/'.$candidate->resume_path }}"
                                                        target="_blank"><img src="{{ asset('img/resume.png') }}" /></a></td>
                                                <td>
                                                    @php
                                            ...
                                            @empty
                                            <tr>
                                                <td class="text-danger text-center" colspan="7">No record(s) found.</td>
                                            </tr>
                                            @endforelse
                                        </tbody>
                                    </table>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        checkboxes.forEach(function(checkbox) {
            checkbox.addEventListener('change', function() {
                ...
                checkAllCheckbox.indeterminate = !allChecked && Array.from(checkboxes).some(checkbox => checkbox.checked);
            });
        });
    });
</script>
