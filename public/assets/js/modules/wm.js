...
$(document).on('click', '.addNewItemBtn', (e) => {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.status == 200) {
                ...
                parentDetails = Array.isArray(data.parentDetails) ? data.parentDetails : [];
                parentHeirarchy = Array.isArray(data.parentHeirarchy) ? data.parentHeirarchy : [];
                ...
                let rowHtml = `
                    <tr>
                        ...
                        <td>
                            <div class="form-check form-check-primary custom-checkbox">
                                <input class="form-check-input" type="checkbox" ${is_last_level === 1 ? 'checked disabled' : ''}>
                                <label class="form-check-label"></label>
                            </div>
                        </td>
                        <td>
                            <div>
                                ...
                                <select class="form-select mw-100 mb-25 parent-dropdown parent_id" name="parent_id">
                                    ${parentOptions}
                                </select>
                            </div>
                        </td>
    ...
    $(".mrntableselectexcel tr").each(function (levelCounter) {
        ...
        $(this).find("input.name").attr("name", `details[${levelCounter + 1}][name]`);

        // Update 'name' for the second checkbox and set the name for storage_point
        let $storagePointCheckbox = $(this).find("input[type='checkbox']").eq(1);
        $storagePointCheckbox.attr("name", `details[${levelCounter + 1}][storage_point]`);
        ...
        $(this).find("input.is_first_level").attr("name", `details[${levelCounter + 1}][is_first_level]`);
        $(this).find("input.is_last_level").attr("name", `details[${levelCounter + 1}][is_last_level]`);
        $(this).find("select.parent_id").attr("name", `details[${levelCounter + 1}][parent_id]`);
        
        // Update 'name' for max_weight and max_volume inputs
        $(this).find("input.max_weight").attr("name", `details[${levelCounter + 1}][max_weight]`);
        $(this).find("input.max_volume").attr("name", `details[${levelCounter + 1}][max_volume]`);
    });

    // Listen for change event on the storage_point checkbox
    $(".mrntableselectexcel").on("change", "input[type='checkbox']", function () {
