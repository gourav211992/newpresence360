...
<tr>
    ...
    @else
        <td class="no-wrap">{{$piItem->po?->book?->book_name ?? ''}}</td>
        <td class="no-wrap">{{$piItem->po?->document_number ?? ''}}</td>
        <td class="no-wrap">{{$piItem->po?->getFormattedDate('document_date') ?? ''}}</td>
        <td class="no-wrap">{{$piItem->item_code ?? ''}}</td>
        <td class="no-wrap">{{$piItem?->item?->item_name}}</td>
        <td>{!! $html !!}</td>
        <td class="no-wrap">{{$piItem?->uom?->name}}</td>
        <td class="text-end">{{$piItem->order_qty}}</td>
        <td class="fw-bolder text-dark">
            {{$piItem?->po?->vendor?->company_name}}
            {{-- <select class="form-select select2" name="vend_name">
                <select class="form-select select2" name="vend_name">
                    @php
                    $approvedVendorIds = \App\Helpers\ItemHelper::getItemApprovedVendors($piItem->item_id, $documentDate)
        ...
        <td class="no-wrap">{{$piItem?->po?->store_location?->store_name ?? ''}}</td>
        <td class="no-wrap">{{$piItem->po?->department?->name ?? ''}}</td>
    @endif
</tr>
@empty
<tr>
