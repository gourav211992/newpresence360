    ...
    <script>
        ...
        function setServiceParameters(parameters) {
            ...
            if(reference_from_service.length) {
                ...
                } else if(reference_from_service.includes(so)) {
                    $("#reference_from").removeClass('d-none');
                } else {
        ...
        $(document).on('click','#deleteBtn', (e) => {
            ...
            if (poItemIds.length) {
                ...
                Swal.fire({
                    title: 'Error!',
                    text: `You cannot delete expense item(s) at row(s): ${rowNumbers}`,
                    icon: 'error',
                });
                return false;
            }
        ...
        $(document).on('click', '#amendmentSubmit', (e) => {
            let actionUrl = "{{ route('expense-adv.amendment.submit', $mrn->id) }}";
            fetch(actionUrl).then(response => {
                return response.json().then(data => {
                    if (data.status == 200) {
                        Swal.fire({
