...
$(document).on('click', '.inspectionChecklistBtn', function () {
    ...
    const existingInput = document.querySelector(`input[id="inspection_data_${rowCount}"]`);
    ...
    let masterChecklist = {}, existingChecklist = [];
    try {
    ...
    try {
        existingChecklist = JSON.parse(rawExistingData || rawTestExisting || '[]') || [];
    ...
    const slug = s => norm(s).toLowerCase().replace(/\s+/g, ' ').replace(/[^\w\s-]/g, '');
    ...
    existingChecklist.forEach(raw => {
        ...
        const cid = norm(raw.checkList_id ?? raw.checklist_id ?? raw.checkListId);
        const did = norm(raw.detail_id ?? raw.checklist_detail_id ?? raw.parameter_item_id);
        ...
        const item = { checklistId: cid, detailId: did, parameterName: pname, parameterValue: pvalue, result, inspId };
            ...
            switch (type) {
                ...
                case 'list':
                    ...
                    (detail.values || []).forEach(opt => {
                        ...
                        html += `<option value="${escapeHTML(ov)}" ${selected}>${escapeHTML(ov)}</option>`;
                    });
                    html += `</select>
                        <div class="invalid-feedback">Required</div>`;
                    break;
