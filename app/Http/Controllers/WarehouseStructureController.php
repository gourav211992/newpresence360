...
class WarehouseStructureController extends Controller
{
                ...
                foreach ($request->all()['levels'] as $l_key => $level) {
                    ...
                    if (isset($level['details'])) {
                        foreach ($level['details'] as $detail) {
                            ...
                            $prDetail = new WhStructureDetail();
    ...
    public function edit($id)
    {
        ...
        $pRoute = WhStructure::with(['levels' => function ($e) {
    ...
    public function update(WhStructureRequest $request, $id)
    {
        ...
        try {
            ...
            if ($deletedData) {
                foreach ($deletedData as $data) {
                    if (@$data['production_level_id']) {
                        if ($data['type'] === 'parent') {
                            WhStructureDetail::where('production_level_id', @$data['production_level_id'])->delete();
                            ProductionLevel::where('id', @$data['production_level_id'])->delete();
                        } elseif ($data['type'] === 'child') {
                            if (!empty(@$data['child'])) {
                                WhStructureDetail::whereIn('id', @$data['child'])->delete();
                            }
                        }
                    }
                }
            ...
            // Level Update
            $prRouteData = WhStructure::find($id);
            if (!$prRouteData) {
                return response()->json([
                    ...
                    if (isset($level['level_id']) && $level['level_id']) {
                        foreach ($level['details'] as $key => $child) {
                            ...
                            if (isset($child['child_id']) && $child['child_id']) {
                                $prDetail = WhStructureDetail::find($child['child_id']);
    ...
    public function delete($id)
    {
        ...
        WhStructureDetail::where('production_route_id', $pRoute->id)->delete();
    ...
    {
        $item = Item::find($request->item_id);
        $prData = WhStructureDetail::where('item_id', $item->id)
