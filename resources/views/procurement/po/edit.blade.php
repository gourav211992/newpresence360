...
$(document).on('click', '.submitAddress', function (e) {
    ...
    var url = '{{ route("po.address.save", ["type" => ":type", "id" => ":id"]) }}'.replace(':type', type).replace(':id', id);
    fetch(url, {
        method: method,
        body: innerFormData,
        headers: {
   ...
   ids = JSON.parse(ids);
    ...
    if(ids.length) {
        ids.forEach((id,index) => {
            ...
            let piItemHiddenId = $(`.form-check-input[data-id='${id}']`).closest('tr').find("input[name*='[pi_item_hidden_ids]']").val();
            if(piItemHiddenId) {
                let idsToRemove = piItemHiddenId.split(',');
                let selectedPiIds = localStorage.getItem('selectedPiIds');
                if(selectedPiIds) {
                    ...
                    $('tr[data-group-item]').each(function () {
                        ...
                        for (let rowItem of data.data.updatedGroupItems) {
                            ...
                            if (
                                ...
                                $(this).find("input[name*='[qty]']").val(Number(rowItem.total_qty).toFixed(2));
    ...
    $.ajax({
        ...
        success: function (response) {
            setTimeout(() => {
                ...
                window.location.href = '{{route("po.index",["type" => ":type"])}}'.replace(':type',type);
            },500);
        },
        error: function (xhr) {
