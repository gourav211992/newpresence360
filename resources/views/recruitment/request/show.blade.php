            ...
            <div class="row">
                <div class="content-header-left col-md-6 mb-2">
                    <div class="row breadcrumbs-top">
                        <div class="col-12">
                            <h2 class="content-header-title float-start mb-0">Job Request View</h2>
                ...
                <div class="content-header-right text-sm-end col-md-6 mb-50 mb-sm-0">
                    <div class="form-group breadcrumb-right">
                        ...
                        @if($jobRequest->status == App\Helpers\CommonHelper::PENDING && $user->id ==
                        $jobRequest->created_by)
                            <button class="btn btn-danger btn-sm mb-50 mb-sm-0" data-bs-target="#status-modal"
                            data-bs-toggle="modal" data-status="{{ App\Helpers\CommonHelper::REVOKED }}" data-title="Revoked Request"><i data-feather="x-circle"></i> Revoke</button>
                        @endif

                        @if(in_array($jobRequest->status, [App\Helpers\CommonHelper::PENDING, App\Helpers\CommonHelper::APPROVED_FORWARD]) && $jobRequest->approval_authority == $user->id)
                            @if($user->manager_id)
                                <button class="btn btn-success btn-sm mb-50 mb-sm-0" data-bs-target="#status-modal" data-bs-toggle="modal" data-status="{{ App\Helpers\CommonHelper::APPROVED_FORWARD }}" data-title="Approved Request" data-id="{{ $jobRequest->id }}">
                                    <i data-feather="check-circle" class="me-50"></i>
                                    <span>Approved</span>
                                </button> 
                            @else
                                <button class="btn btn-success btn-sm mb-50 mb-sm-0" data-bs-target="#status-modal" data-bs-toggle="modal" data-status="{{ App\Helpers\CommonHelper::FINAL_APPROVED }}" data-title="Approved Request" data-id="{{ $jobRequest->id }}">
                                    <i data-feather="check-circle" class="me-50"></i>
                                    <span>Final Approved</span>
                                </button> 
                            @endif
                            <button class="btn btn-danger btn-sm mb-50 mb-sm-0" data-bs-target="#status-modal" data-bs-toggle="modal" data-bs-toggle="modal" data-status="{{ App\Helpers\CommonHelper::REJECTED }}" data-title="Reject Request" data-id="{{ $jobRequest->id }}"><i data-feather="x-circle"></i> Reject</button>
                        @endif
                    </div>
                </div>
            </div>
                                        ...
                                        <div class="row">
                                            <div class="col-md-7">
                                                <div class="new-applicayion-help-txt pb-25">
                                                    <h4 class="purchase-head text-start">{{ $jobRequest->job_title_name
                                                        }}</h4>
                                            ...
                                            <div class="col-md-5 text-end d-sm-flex flex-sm-column align-items-sm-end">
                                                @php
                                                if(in_array($jobRequest->status, [App\Helpers\CommonHelper::PENDING,App\Helpers\CommonHelper::REJECTED])){
                                                    $className = 'badge-light-danger';
                                                }else if($jobRequest->status == App\Helpers\CommonHelper::FINAL_APPROVED){
                                                    $className = 'badge-light-success';
                                                }else if($jobRequest->status == App\Helpers\CommonHelper::APPROVED_FORWARD){
                                                ...
                                                @endphp
                                                <span
                                                    class="badge rounded-pill fw-bold px-2 py-50 {{ $className }} rounded"><i
                                                        data-feather="check-circle"></i> {{ ucfirst($jobRequest->status)
                                                    }}</span>
                                            </div>
                                            <div class="col-md-12">
                                            ...
                                            <div class="row">
                                                <div class="col-md-6 mb-50">
                                                    ...
                                                    <h6 class="fw-bolder text-dark"><span
                                                            class="badge rounded-pill badge-light-success badgeborder-radius font-small-2">{{
                                                            ucfirst($jobRequest->job_type) }}</span>
                                                    </h6>
                                                </div>
                                                <div class="col-md-6 mb-50">
                                                    <label class="form-label">Priority </label>
                                                    @php
                                                    if($jobRequest->priority == App\Helpers\CommonHelper::HIGH){
                                                    $className = 'badge-light-danger';
                                                    }else if($jobRequest->priority ==
                                                    ...
                                                    @endphp
                                                    <h6 class="fw-bolder text-dark"><span
                                                            class="badge rounded-pill {{ $className }} badgeborder-radius font-small-2">{{
                                                            ucfirst($jobRequest->priority) }}</span>
                                                    </h6>
                                                </div>
                                                <div class="col-md-6 mb-50">
                                                    <label class="form-label">No. of Position</label>
                                                    <h6 class="fw-bolder text-dark">{{ $jobRequest->no_of_position
                                                        }}</span></h6>
                                                </div>
                                                <div class="col-md-6 mb-50">
                                                ...
                                                @if($jobRequest->certification_name)
                                                <div class="col-md-6 mb-50">
                                                    <label class="form-label">Certification </label>
                                                    <h6 class="fw-bolder text-dark">{{ $jobRequest->certification_name
                                                        }}</h6>
                                                </div>
                                                @endif
                                                ...
                                                <div class="col-md-6 mb-50">
                                                    ...
                                                    <h6 class="fw-bolder text-dark">{{ $jobRequest->work_experience_name
                                                        }}</h6>
                                                </div>
                                                <div class="col-md-6 mb-50">
                                                    <label class="form-label">Expected D.O.J</label>
                                                    <h6 class="fw-bolder text-dark">{{ $jobRequest->expected_doj ?
                                                        App\Helpers\CommonHelper::dateFormat($jobRequest->expected_doj)
                                                        : '' }}</h6>
                                                </div>

                                                <div class="col-md-6 mb-50">
                                                    <label class="form-label">Job Location </label>
                                                    <h6 class="fw-bolder text-dark">{{ $jobRequest->placed_location_name
                                                        }}</h6>
                                                </div>
                                                ...
                                                <div class="col-md-12 mb-50">
                                                    ...
                                                    <h6 class="fw-bolder text-dark">
                                                        ...
                                                        <span
                                                            class="badge rounded-pill badge-light-secondary badgeborder-radius font-small-2">{{
                                                            $skill }}</span>
                                                        @empty
                                                        @endforelse
                                                    </h6>
                                    ...
                                    <div class="card-body customernewsection-form">
                                        ...
                                        <div
                                            ...
                                            <ul class="timeline ms-50 newdashtimline ">
                                                ...
                                                @php
                                                    if(in_array($log->status, [App\Helpers\CommonHelper::PENDING,App\Helpers\CommonHelper::REJECTED])){
                                                        $logClassName = 'badge-light-danger';
                                                        $timeLineClass = 'timeline-point-danger';
                                                    }else if($log->status == App\Helpers\CommonHelper::FINAL_APPROVED){
                                                        $logClassName = 'badge-light-success';
                                                        $timeLineClass = 'timeline-point-success';
                                                    }else if($log->status == App\Helpers\CommonHelper::APPROVED_FORWARD){
                                                ...
                                                <li class="timeline-item">
                                                    ...
                                                    <div class="timeline-event">
                                                        <div
                                                            ...
                                                            <span class="badge rounded-pill {{ $logClassName }}">{{ ucwords(str_replace('-',' ',$log->status)) }}</span>
                                                        </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title fw-bolder text-dark namefont-sizenewmodal" id="jobDescModalLabel">Job Description
                </h5>
...
<div class="modal fade" id="status-modal" tabindex="-1" aria-labelledby="statusModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form role="post-data" method="POST" action="{{ route('recruitment.requests.update-status',['id' => $jobRequest->id]) }}" redirect="{{route('recruitment.requests.show',['id' => $jobRequest->id]) }}">
                <div class="modal-body pb-2">
                        <div class="row mt-1">
                            <div class="col-md-12">
                                <div class="mb-1">
