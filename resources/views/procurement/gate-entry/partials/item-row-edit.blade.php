@foreach($mrn->items as $key => $item)
   ...
   <tr id="row_{{$rowCount}}" data-index="{{$rowCount}}" @if($rowCount < 2 ) class="trselected" @endif>
      ...
      <input type="hidden" name="components[{{$rowCount}}][purchase_order_id]" value="{{$item?->gateEntryHeader?->purchase_order_id}}">
      @if($item->poItem?->po?->type == 'supplier-invoice')
         <input type="hidden" name="components[{{$rowCount}}][po_detail_id]" value="{{$item->poItem?->po_item?->id}}">
         <input type="hidden" name="components[{{$rowCount}}][supplier_inv_detail_id]" value="{{$item->poItem?->id}}">
      @else
         <input type="hidden" name="components[{{$rowCount}}][po_detail_id]" value="{{$item->poItem?->id}}">
