    ...
    <div class="app-content content ">
        ...
        <div class="content-wrapper container-xxl p-0">
                    ...
                    <div class="content-header-right text-sm-end col-md-6 mb-50 mb-sm-0">
                        <div class="form-group breadcrumb-right">
                            ...
                            <button onClick="javascript: history.go(-1)" class="btn btn-secondary btn-sm mb-50 mb-sm-0"><i
                                ...
                                <a href="{{ route('loanAppraisal.create', $termLoan->id) }}"
            ...
            <div class="content-body">
                ...
                @endif
                    @error('remarks')
                    <div class="text-danger">{{ $message . ' Remarks' }}</div>
                    @enderror
                                    ...
                                    <div class="card-body customernewsection-form">
                                        ...
                                        <div class="text-center new-applicayion-help-txt  mb-1 pb-25">
                                            ...
                                            <h6 class="mt-2">({{ $termLoan->name ?? 'N/A' }} |
                                                {{ $overview->term_loan ?? $termLoan->loan_amount }} |
                                                {{ $overview->updated_at ?? $termLoan->created_at ? $termLoan->created_at->format('d-m-Y') : 'N/A' }})
                                            </h6>
                                        </div>
                                                    ...
                                                    <div class="col-md-7">
                                                        ...
                                                        <div class="step-custhomapp bg-light mb-0">
                                                            @php
                                                                ...
                                                                if ($termLoan->approvalStatus == 'submitted') {
                                                                    $activeStatuses = []; // All statuses inactive
                                                                } elseif (array_key_exists($termLoan->approvalStatus, $specialStatuses)) {
                                                                    $activeStatuses = $specialStatuses[$termLoan->approvalStatus];
                                                                } else {
                                                                    $currentStatusIndex = $statuses[$termLoan->approvalStatus] ?? count($statuses);
                                                            ...
                                                            <ul class="nav nav-tabs mb-0 mt-25 custapploannav customrapplicationstatus" role="tablist">
                                                                @foreach ($statuses as $status => $index)
                                                                    <li class="nav-item">
                                                                        ...
                                                                        <a class="nav-link {{ $termLoan->approvalStatus == $status ? 'active' : '' }}"
...
{{--                    <div>--}}
{{--                        <h4 class="modal-title fw-bolder text-dark namefont-sizenewmodal" id="myModalLabel17">Approve Term--}}
{{--                            Loan Application</h4>--}}
                    ...
                    <div class="modal-header">
                        <div>
                            <h4 class="modal-title fw-bolder text-dark namefont-sizenewmodal" id="myModalLabel17">Return
                                Home
                                Loan Application</h4>
                            <p class="mb-0 fw-bold voucehrinvocetxt mt-0">({{ $termLoan->name ?? 'N/A' }} |
                                {{ $overview->term_loan ?? $termLoan->loan_amount }} |
                                {{ $overview->updated_at ?? $termLoan->created_at ? $termLoan->created_at->format('d-m-Y') : 'N/A' }})
                            </p>
                        </div>
                            ...
                            <div class="col-md-12">
                                ...
                                <input type="hidden" name="loan_application_id"
                                    ...
                                    <div class="mb-1">
                                        ...
                                        <select class="form-select select2 user-behalf-home-loan" multiple name="behalf_users[]" required>
                                            @foreach ($behalf_users as $behalf_user)
                                            <option value="{{$behalf_user->name}}">{{$behalf_user->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    @endif
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            checkboxes.forEach(function(checkbox) {
                if (checkbox.hasAttribute('readonly')) {
                    console.log('Checkbox has readonly attribute:', checkbox);
                    checkbox.removeAttribute('readonly');
                    console.log('Removed readonly from:', checkbox);
                } else {
                    console.log('Checkbox does not have readonly attribute:', checkbox);
                }

                if (checkbox.hasAttribute('disabled')) {
                    console.log('Checkbox has disabled attribute:', checkbox);
                    checkbox.removeAttribute('disabled');
                    console.log('Removed disabled from:', checkbox);
                } else {
                    console.log('Checkbox does not have disabled attribute:', checkbox);
                }
            });
            ...
            for (let i = 0; i < textareas.length; i++) {
                if (textareas[i].hasAttribute('disabled')) {
                    console.log('Textarea has disabled attribute:', textareas[i]);
                    textareas[i].removeAttribute('disabled');
                    console.log('Removed disabled from:', textareas[i]);
                } else {
                    console.log('Textarea does not have disabled attribute:', textareas[i]);
                }
            }

            // Mutation Observer to monitor attribute changes
                ...
                for (let mutation of mutationsList) {
                    if (mutation.type === 'attributes') {
                        ...
                        if (mutation.attributeName === 'readonly' || mutation.attributeName === 'disabled') {
                            ...
                            console.log('Removed attribute:', mutation.attributeName, 'from:', mutation.target);
                        }
                    }
                }
            });
        ...
        function onPostVoucherOpen(type = "not_posted")
{
    ...
    $.ajax({
        url: apiURL + "?book_id=" + "{{$termLoan->book_id}}" + "&document_id=" + "{{$termLoan->id}}"+"&remakrs="+remakrs+"&data="+formData+"&loan_data="+loanData,
       type: "GET",
        dataType: "json",
        success: function(data) {
            if (!data.data.status) {
...
{
    const bookId = "{{isset($termLoan) ? $termLoan -> book_id : ''}}";
    const documentId = "{{isset($termLoan) ? $termLoan -> id : ''}}";
